...
namespace Nomade.Efact.LogNegocio
{
    public class nEFBajaFactura
    {
        ...
        public string fnGetFactura(string p_CTLG_CODE, string p_VTAC_CODE)
        {
            try
            {
                cEFBajaFactura ocEFBajaFactura = new cEFBajaFactura("Bn");

                // Inicio - ID del Documento
                DataTable oDT_IdDoc = ocEFBajaFactura.fnListarID(p_CTLG_CODE, p_VTAC_CODE);
               ...
               // s1C = p_VTAC_CODE;
                ...
                DataTable oDT_DatosEmpresa = ocEFBajaFactura.fnListarDatosEmpresa(p_CTLG_CODE);
                ...
                DataTable oDT_DatosDoc = ocEFBajaFactura.fnListarDatosDoc(p_CTLG_CODE, p_VTAC_CODE);
                ...
                string sNombreArchivo = sPath + @"baja\invoice\" + "17" + p_VTAC_CODE + ".csv";
        ...
        public string fnInsertarDocBaja(string p_CTLG_CODE, string p_VTAC_CODE)
        {
            try
            {
                ...
                DataTable oDT_IdDoc = ocEFBajaFactura.fnInsertarBaja(p_CTLG_CODE, p_VTAC_CODE);
        ...
        public string fnGetFacturaOrbitum(string p_CTLG_CODE, string p_VTAC_CODE)
        {
            try
            {
                cEFBajaFactura ocEFBajaFactura = new cEFBajaFactura("Bn");
                ...
                // Inicio - Datos del Documento
                DataTable oDT_DatosDoc = ocEFBajaFactura.fnListarDatosComBaja(p_CTLG_CODE, p_VTAC_CODE);
        ...
        public string fnVerificarBajaDocOrbitum(string p_CTLG_CODE, string p_VTAC_CODE)
        {
            ...
            try
            {
                ...
                {
                    sRespuesta = ocEFBajaFactura.fnActualizar_ELECT_IND_VTA(p_CTLG_CODE, p_VTAC_CODE, "B");
                    if (sRespuesta == "S")
                    {
                ...
                else if (!File.Exists(sNombreArchivoEnvio) && !File.Exists(sNombreArchivoResp))
                {
                    ...
                    {
                        sRespuesta = ocEFBajaFactura.fnActualizar_ELECT_IND_VTA(p_CTLG_CODE, p_VTAC_CODE, "Q");
                        if (sRespuesta == "S")
                        {
                    ...
                    else
                    {
                        ...
                        {
                            sRespuesta = ocEFBajaFactura.fnActualizar_ELECT_IND_VTA(p_CTLG_CODE, p_VTAC_CODE, "X");
                            if (sRespuesta == "S")
                            {
                ...
                {
                    sRespuesta = ocEFBajaFactura.fnActualizar_ELECT_IND_VTA(p_CTLG_CODE, p_VTAC_CODE, "S");
                    if (sRespuesta == "S")
                    {
