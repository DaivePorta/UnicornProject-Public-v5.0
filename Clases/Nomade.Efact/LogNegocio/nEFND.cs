	...
	public class nEFND
	{
		...
		public void fnGetND(string p_CTLG_CODE, string p_ND_CODE)
		{
			try
			{
				...
				string sNombreArchivo = In_LocalPathEfact + FnGetNombreArchivo(p_CTLG_CODE, "08", s1B, "csv");
		...
		private string FnGetNombreArchivo(string p_CTLG_CODE, string p_Tipo_Doc, string p_Serie_Y_Numero, string p_Extension_Sin_Punto)
		{
			...
			if (oDT_DatosEmpresa != null)
			{
				DataRow oDR_DatosEmpresa = oDT_DatosEmpresa.NewRow();
				oDR_DatosEmpresa = oDT_DatosEmpresa.Rows[0];
				rucEmpresa = oDR_DatosEmpresa["3C"].ToString(); // Nro de RUC de la Empresa   
			}
		...
		public string fnVerificarDoc(string p_CTLG_CODE, string p_ND_CODE)
		{
			...
			try
			{
				...
				string sNombreArchivo = Out_LocalPathEfact + FnGetNombreArchivo(p_CTLG_CODE, "08", sSerieNroDoc, "xml");
				if (File.Exists(sNombreArchivo))
				{
				...
				{
					sNombreArchivo = In_LocalPathEfact + FnGetNombreArchivo(p_CTLG_CODE, "08", sSerieNroDoc, "csv");
					if (File.Exists(sNombreArchivo))
					{
						sRespuesta = ocEFND.fnActualizar_ELECT_IND_ND(p_CTLG_CODE, p_ND_CODE, "P");
					}
					...
					{
						sNombreArchivo = Error_LocalPathEfact + FnGetNombreArchivo(p_CTLG_CODE, "08", sSerieNroDoc, "csv");
						if (File.Exists(sNombreArchivo))
						{
