...
var CargarDatosCobranzas = function () {
    ...
    var jqxhr = $.ajax({
        type: "POST",
        url: "vistas/cc/ajax/ccmprog.ashx?OPCION=1",
        contentType: false,
        async: false,
        data: data,
        processData: false,
...
function GrabarPostergacion() {
    ...
    if (vErrors(["txtHasta", "txtHora"])) {
      ...
      //  if (parseInt($("#txtDias").val()) < 0) {
        if (dias < 0) {
            alertCustom("La nueva fecha de cobranza no puede ser menor a la  Fecha Actual " +fechaActual2)
            ...
            var jqxhr = $.ajax({
                type: "POST",
                url: "vistas/cc/ajax/ccmprog.ashx?OPCION=2",
                contentType: false,
                data: data,
                processData: false,
                cache: false
...
function MostrarPostergar(event) {
    ...
    ' </button>' +
    '        <h4 id="myModalLabel">POSTERGAR COBRO</h4>' +
    '    </div>' +
