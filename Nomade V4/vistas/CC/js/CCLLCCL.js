...
var CCMLCCL = function () {
    ...
    function fillClientes() {
        ...
        $.ajax({
            type: "post",
            url: 'vistas/cc/ajax/CCLLCCL.ashx?OPCION=E&empresa=' + $('#cboEmpresas').val(),
            contenttype: "application/json;",
            datatype: "json",
            async: false,
            success: function (datos) {
                $('#cboClientes').empty();
                $('#cboClientes').append('<option></option>');
                $('#cboClientes').select2('val', '');
                if (datos != null) {
                    for (var i = 0; i < datos.length; i++) {
                        $('#cboClientes').append('<option value="' + datos[i].PIDM + '">' + datos[i].RAZON_SOCIAL + '</option>');
                    }
                } else {
                    $('#cboClientes').select2('val', '');
                }            
    ...
    function CargarGrilla(oTable) {
        //alert($('#hfPIDM').val());
        $.ajax({
            url: "vistas/cc/ajax/CCLLCCL.ASHX?OPCION=1&Proveedor=" + $('#cboClientes').val() + '&empresa=' + $('#cboEmpresas').val() + '&filter="C"',
            contenttype: "application/json;",
            datatype: "json",
            async: false,
            success: function (datos) {
    ...
    function adicionarow(oTable) {
        ...
        $.ajax({
            type: "post",
            url: "vistas/cc/ajax/CCLLCCL.ASHX?OPCION=2&Proveedor=" + $('#cboClientes').val() + '&empresa=' + $('#cboEmpresas').val(),
            contenttype: "application/json;",
            datatype: "json",
            async: false,
            success: function (datos) {
    ...
    function editRow(oTable, nRow) {
        ...
        $.ajax({
            type: "post",
            url: "vistas/cc/ajax/CCLLCCL.ashx?OPCION=3",
            contenttype: "application/json;",
            datatype: "json",
            async: false,
            success: function (datos) {
        ...
        jqTds[6].innerHTML = '<input type="text" readonly=”readonly” class="m-wrap small required fecha1"  value="' + aData.FECHA_INICIO + '">';
    ...
    var CargaInicial = function () {
        ...
        var tbandeja1 = $('#tblBandeja').dataTable(parms);
        ...
        $('#agregar').on('click', function () {
            ...
            if ($('#cboClientes').val().length != 0)
            { adicionarow(tbandeja1); }
            else
            { alertCustom('Debe seleccionar un Cliente'); }
        ...
        $('#cboClientes').on('change', function () {
            //if ($('#cboClientes').val() != personaAnterior) {
                ...
                personaAnterior = $('#cboClientes').val();
            //} else {
            //    personaAnterior = "";
            //}
        });
        ...
        $('#tblBandeja a.delete').live('click', function (e) {
            ...
            objectRowDelete = $(this).parents('tr')[0];

            console.log(objectRowDelete);
            
        });

        $("#btnQuitarRow").on('click', function () {
            //var nRow = $(this).parents('tr')[0];
            tbandeja1.fnDeleteRow(objectRowDelete);
            $('#agregar').attr('disabled', false);           
            $("#ModalConfirmacion").modal("hide");
        });

        $("#btnCancelarRow").on('click', function () {
            $("#ModalConfirmacion").modal("hide");
        });
        ...
        $('#tblBandeja a.edit').live('click', function (e) {
            ...
            if (bValidacion === 0) {
                GrabarRow($('#cboClientes').val(), aData.SECUENCIA, aData.LINCRED, aData.MONEDA, aData.PLAZO, aData.FECHA_INICIO, aData.FECHA_FINAL, aData.ESTADO, aData.USUARIO, $('#cboEmpresas').val())
                $('#agregar').removeAttr('disabled');
        ...
        function GrabarRow(Cliente, secuencia, lineacredito, moneda, plazo,
                           fecini, fecfin, estado, usuario, empresa) {
            Bloquear("ventana");
            $.ajaxSetup({ async: false });
            $.post("vistas/cc/ajax/CCLLCCL.ASHX", {
                opcion: 'I',
                proveedor: Cliente,
                secuencia: secuencia,
        ...
        if (cod != undefined) {
            $('#cboEmpresas').select2('val',emp);
            $('#cboClientes').select2('val', cod);
            $('#cboClientes').change();
        }
    }  
...
var CCLLCCL = function () {
    ...
    var CargaInicial = function () {
        ...
        $('#tblBandeja tbody').on('click', 'tr', function () {
            ...
            //console.log(row.ESTADO_CLIENTE);

            if (row.ESTADO_CLIENTE == 'A') {
                if ($(this).hasClass('selected')) {
                    ...
                    window.location.href = '?f=CCMLCCL&codigo=' + code + '&empresa=' + empresa;
                }
            } else {
                infoCustom2("El Cliente seleccionado está INACTIVO");
            }
    ...
    function fcargarbandeja(oTable) {
        ...
        //Cargar monedas
        $.ajax({
            url: "vistas/CC/ajax/CCLLCCL.ASHX?OPCION=5&empresa=" + $('#cboEmpresas').val(),
            contenttype: "application/json;",
            datatype: "json",
            async: false,
            success: function (datos) {
        ...
        $.ajax({
            url: "vistas/cc/ajax/CCLLCCL.ASHX?OPCION=4&empresa=" + $('#cboEmpresas').val(),
            contenttype: "application/json;",
            datatype: "json",
            async: false,
            success: function (datos) {
