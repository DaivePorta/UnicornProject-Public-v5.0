...
function CargarTablaPrecios() {
    ...
    if (continuar) {
        ...
        $.ajax({
            type: "POST",
            url: "vistas/nv/ajax/nvmprec.ashx?OPCION=1&CTLG=" + $.trim($('#cboEmpresa').val()) +
            "&SCSL=" + sucursal +
...
function ActualizarPrecio(input) {
    ...
    if (vErrors(['cboEmpresa'])) {
        ...
        var jqxhr = $.ajax({
            type: "POST",
            url: "vistas/NV/ajax/NVMPREC.ASHX",
            contentType: false,
            data: data,
            processData: false,
            cache: false,
...
function CalcularPreciosCantidad(cod, mone) {
    precAnterior = "";
    utilAnterior = "";
    var f = "f_" + cod
    var idCosto;
...
function ImprimirListaPrecios() {
    ...
    if (vErrors(['cboSucursal', 'cboEmpresa'])) {
        ...
        if (continuar) {
            ...
            $.ajax({
                type: "POST",
                url: "vistas/nv/ajax/nvmprec.ashx?OPCION=5&CTLG=" + $.trim($('#cboEmpresa').val()) +
                "&SCSL=" + sucursal +
                "&GRUP_CODE=" + $('#cboGrupo').val() +
                "&SUBGRUP_CODE=" + $('#cbosubgrupo').val() +
                "&PRECIO_FILTRO=" + sTipoPrecio +
