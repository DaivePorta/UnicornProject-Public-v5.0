...
function setSeleccionDocumento(nro_atencion, cliente, pidm_abogado, valor) {//POR EL MOMENTO TODOS LOS SERVICIOS ESTÁN EN MONEDA SOLES
...
function cargarParametrosSistema() {
    let filtro = "DIGP,REDN,DETR,RETN,RETR,IMRE,BFDV,ODON,ADET,ACON,CURS,MIGO,0021"; //Aquí van los parámetros que se van a utilizar en la pantalla y luego se le hace su case
...
function CompletaryPagar() {
    ...
    if ($('#txtEfectivo2').val() != "") {
        if ($("#cboMedioPago2").val() == "0020" && $("#txtNroOpe2").val().length < 16) { //DPORTA 09/12/2021 BILLETERA DIG.
            if ($("#txtNroOpe2").val().length == 0) {
                vErrors(['txtNroOpe2']);
            } else {
                infoCustom2("Debe colocar el número de celular correctamente!");
                $("#txtNroOpe2").pulsate({
    ...
    if ($('#txtEfectivo3').val() != "") {
        if ($("#cboMedioPago3").val() == "0020" && $("#txtNroOpe3").val().length < 16) { //DPORTA 09/12/2021 BILLETERA DIG.
            if ($("#txtNroOpe3").val().length == 0) {
                vErrors(['txtNroOpe3']);
            } else {
                infoCustom2("Debe colocar el número de celular correctamente!");
                $("#txtNroOpe3").pulsate({
    ...
    if ($("#txtNumDctoComp").val() == "") {//INSERTA
        ...
        if (continuar) {//DPORTA 21/04/2021
            if ($("#cboMedioPago").val() == '0001' || $("#cboMedioPago").val() == '0003' || $("#cboMedioPago").val() == '0005' || $("#cboMedioPago").val() == '0006' ||
                $("#cboMedioPago2").val() == '0001' || $("#cboMedioPago2").val() == '0003' || $("#cboMedioPago2").val() == '0005' || $("#cboMedioPago2").val() == '0006' ||
                $("#cboMedioPago3").val() == '0001' || $("#cboMedioPago3").val() == '0003' || $("#cboMedioPago3").val() == '0005' || $("#cboMedioPago3").val() == '0006') {

                verificaNroOpera = verificarNroOperacion($("#cbo_OrigenPago").val().substring(0, 1) + '-' + $("#txtNroOpe").val(),
                    $("#cbo_OrigenPago2").val().substring(0, 1) + '-' + $("#txtNroOpe2").val(),
                    $("#cbo_OrigenPago3").val().substring(0, 1) + '-' + $("#txtNroOpe3").val());
        ...
        if (continuar) {
            ...
            if (detallesVenta.length != 0) {
                ...
                data.append('p_CMNT_DCTO', ($("#txt_comentario").val() == "" || $("#txt_comentario").val().length == 0) ? "Orden de Servicio" : $("#txt_comentario").val());
...
function ActualizaPrecioEstandarDetalle(campo, valor, indice) {
    if ($("#hfCompletoInd").val() == "S") {
        ...
        if (parseFloat($(campo).val()) < parseFloat(precioMinimo) || $(campo).val().trim() == "") {
            ...
            //DPORTA
            var totalBruto = (parseFloat(detallesVenta[indice].CANTIDAD)) * parseFloat(precioMinimo);
            var montoDescuento = 0;

            /*00*/detallesVenta[indice].PRECIO_DETALLE = parseFloat(precioMinimo).toFixed(2);
            /*01*/detallesVenta[indice].TOTAL_BRUTO = totalBruto.toFixed(2);
            /*02*/detallesVenta[indice].MONTO_DESCUENTO = montoDescuento.toFixed(2);
            /*03*/detallesVenta[indice].TOTAL_NETO = (totalBruto - montoDescuento).toFixed(2);
            ListarTablaDetalles(ObtenerTablaDetalles());
                ...
                switch (MedioActual2) {
                    ...
                    case "0020"://OTROS: YAPE, PLIN, TUNKI, ETC BILLETERA DIG.
                        ...
                        if (billetera_dig == 'S') {
                            ...
                            $("#lbl_detalle4_2").html("App - Nro. Celular");
                            $("#txtDestino2").parent().html("<select id='cbDestino2' class='obligatorio span12 cbocta2' data-placeholder='CUENTA DE CLIENTE'></select>");
                ...
                switch (MedioActual3) {
                    ...
                    case "0020"://OTROS: YAPE, PLIN, TUNKI, ETC BILLETERA DIG.
                        ...
                        if (billetera_dig == 'S') {
                            ...
                            $("#lbl_detalle4_3").html("App - Nro. Celular");
                            $("#txtDestino3").parent().html("<select id='cbDestino3' class='obligatorio span12 cbocta3' data-placeholder='CUENTA DE CLIENTE'></select>");
...
function CargarDatosCobroPorDefecto() {
    ...
    $('#cbo_OrigenPago, #cbo_OrigenPago2').select2("val", "Caja").change();
