var NVLORAT = function () {
    ...
    var fillBandejaPersonas = function () {
        ...
        var parms = {
            ...
            columns: [
                ...
                },
                {
                    ...
                    createdCell: function (td, cellData, rowData, row, col) {
                        $(td).attr('align', 'center');
                        if (rowData.ESTADO == "PENDIENTE" || rowData.ESTADO == "ANULADO") {
                            $(td).html('<a class="btn green"><i class="icon-refresh"></i></a>');
                        } else {
                            $(td).html("");
                        }                       
                    }
        ...
        oTablePersonas = iniciaTabla('tblAtenciones', parms);
        //actualizarEstilos();
        //DPORTA
        $('#tblAtenciones tbody').on('click', 'a', function () {
            $(this).parent().parent().addClass('selected');
            var pos = $('#tblAtenciones').DataTable().row($(this).parent().parent()).index();
            var row = $('#tblAtenciones').DataTable().row(pos).data();
            var code = row.NRO_ATENCION;

            Bloquear("ventana");
            $.ajaxSetup({ async: false });
            $.post("vistas/NV/ajax/NVLORAT.ASHX",
                { OPCION: 'CAMBIO_ESTADO', NRO_ATENCION: code, USUA_ID: $('#ctl00_txtus').val(), CTLG_CODE: $('#ctl00_hddctlg').val() },
                function (res) {
                    Desbloquear("ventana");
                    if (res !== null) {
                        res = (res === 'A') ? 'ANULADO' : 'PENDIENTE';
                        //if (res === 'A') {
                        //    res = 'ANULADO'
                        //}
                        $('#tblAtenciones').DataTable().cell(pos, 6).data(res).draw();
                        exito();
                    } else { noexito(); }
                });
        ...
        //FIN
        $("#tblAtenciones").removeAttr("style");

    }

    var obtenerDocumentos = function (CTLG_CODE, SCSL_CODE, VENDEDOR, ESTADO, DESDE, HASTA) {
        $.ajax({
            type: "post",
            url: "vistas/nv/ajax/NVLORAT.ashx?OPCION=LORDENES&CTLG_CODE=" + CTLG_CODE + "&SCSL_CODE=" + SCSL_CODE + "&VENDEDOR=" + VENDEDOR + "&ESTADO=" + ESTADO + "&DESDE=" + DESDE + "&HASTA=" + HASTA,
            contenttype: "application/json;",
            datatype: "json",
            beforeSend: function () { Bloquear($($("#tblAtenciones").parents("div")[0])); },
            async: true,
            success: function (datos) {
            ...
            error: function (msg) {
                alertCustom("Ordenes no se listaron correctamente");
            },
            complete: function () { Desbloquear($($("#tblAtenciones").parents("div")[0])); }
        });
    ...
    //    var VENDEDOR = ($("#cboVendedor").val() == "TODOS") ? '' : $("#cboVendedor").val();//$("#cboVendedor :selected").text();
    ...
    //    var jqxhr = $.ajax({
    //        type: "POST",
    //        url: "vistas/NV/ajax/NVLORAT.ashx?OPCION=LORDENES",
    //        contentType: false,
    //        data: data,
    //        processData: false,
    //        cache: false
    ...
    function cargainicial() {
        ...
        $("#btnBuscarDoc").on("click", function () {
            if ($("#txtDesde").val().trim() == "" && $("#txtHasta").val().trim() == "") {
                obtenerDocumentos($("#cboEmpresa").val(), $("#cboEstablecimiento").val(), ($("#cboVendedor").val() == "TODOS") ? '' : $("#cboVendedor").val(), ($("#cboEstado").val() == "TODOS") ? '' : $("#cboEstado").val(), $("#txtDesde").val(), $("#txtHasta").val());
...
function cambioEstado(nro_atencion) {
    Bloquear("ventana");
    $.ajaxSetup({ async: false });
    $.post("vistas/NV/ajax/NVLORAT.ASHX",
        { OPCION: 'CAMBIO_ESTADO', NRO_ATENCION: nro_atencion, USUA_ID: $('#ctl00_txtus').val(), CTLG_CODE: $('#ctl00_hddctlg').val() },
        function (res) {
            Desbloquear("ventana");
            if (res !== null) {
                //res = (res === 'A') ? 'ANULADO' : 'ACTIVO';
                if (res === 'A') {
                    res = 'ANULADO'
                }
                //$("#tblDocumento").DataTable().cell(pos, 7).data(res).draw();
                exito();
            } else { noexito(); }
        });
    $.ajaxSetup({ async: true });
...
//function imprimirListaOrdenesAtencion() {
//    //Bloquear("ventana")
//    var data = new FormData();
//    var CTLG_CODE = $("#cboEmpresa").val();
//    var SCSL_CODE = $("#cboEstablecimiento").val();
...
//    var jqxhr = $.ajax({
//        type: "POST",
//        url: "vistas/NV/ajax/NVLORAT.ashx?OPCION=5.5",
//        async: false,
//        contentType: false,
//        data: data,
//        processData: false,
