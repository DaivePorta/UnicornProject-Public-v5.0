...
var NVMDEBC = function () { //NVMNDAC
    ...
    var fillTblDocumentosRef = function () {
        $.ajax({
            type: "post",
            url: "vistas/nv/ajax/NVMDEBC.ashx?OPCION=1" +
    ...
    var cargaInicial = function () {
        ...
        if (cod != null) {
            ...
            var jqxhr = $.ajax({
                type: "POST",
                url: "vistas/nv/ajax/NVMDEBC.ashx?OPCION=4",
                contentType: false,
                data: data,
                processData: false,
                cache: false,
                ...
                .success(function (datos) {
                    ...
                    if (datos != null && datos.length > 0) {
                        ...
                        var jqxhr = $.ajax({
                            type: "POST",
                            url: "vistas/nv/ajax/NVMDEBC.ashx?OPCION=5",
                            contentType: false,
                            data: data2,
                            processData: false,
                            async: true
...
function ValidarTotales() {
    if ($("#cboMotivo").val() == "03") {
        ...
        if (sumt + parseFloat($("#txtPrecioUnitario").val()) > parseFloat($("#txtMonto").val())) {
            parseFloat($("#txtPrecioUnitario").val(parseFloat($("#txtMonto").val() - sumt)))
...
function AgregarDetalle() {
    if ($("#cboMoneda").val() != "") {
        ...
        if (vErrors(['txtDesc', 'txtCantidad', 'cboAfectacionIgv', 'txtPrecioUnitario', 'txtSubtotalItem'])) {
            ...
            //DPORTA
            let subtotalito = $("#txtSubtotalItem").val();
            let montito = $("#txtMonto").val();
                ...
                if (parseFloat(iPrec) > 0) {
                    if (parseFloat(iSubt) > 0) {
                        if ($("#cboMotivo").val() != "03") {
                            ...
                            if (parseFloat(subtotalito) <= parseFloat(montito)) {
                                continuar = true;
                            } else {
                                alertCustom("El Monto de Valor Venta no puede ser mayor al monto de la Nota de Crédito que se desea anular");
                            }
                        }
                    } else {
...
function GrabarNotaDebito() {
    ...
    if (continuar) {
        ...
        var jqxhr = $.ajax({
            type: "POST",
            url: "vistas/nv/ajax/NVMDEBC.ashx?OPCION=3",
            contentType: false,
            data: data,
            processData: false,
            cache: false,
...
//FALTA IMPLEMENTAR PARA LA NOTA DE DÉCITO, ESTA ES DE LA NOTA DE CRÉDITO
    ...
    var IniciaTabla = function () {
        ...
        $('#tblDatos tbody').on('click', 'tr', function () {
            if ($(this).hasClass('selected')) {
                ...
                var code = row.CODIGO;
                window.location.href = '?f=NVMDEBC&codigo=' + code;
            }
        });
