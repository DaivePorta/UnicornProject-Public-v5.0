...
var NVMNDDP = function () {
                ...
                if (datos != null && datos != "" && typeof datos[0].SERIE != "undefined") {
                    ...
                    if (typeof (code) == "undefined") {
                        if ($("#txtSerieNota").val() == "" || $("#txtNroNota").val() == "") {
                            alertCustom("Verifique la numeración de Nota de Débito. Haga clic <a style='color:#35aa47;' href='?f=NCLAUTD'>aquí</a>.")
                        }
                    }
                }
            },
    ...
    var eventoControles = function () {
        ...
        $("#btnBuscarDocumento").on("click", function () {
            if (vErrors(['cboEmpresa', 'cboEstablecimiento', 'txtrazsocial', 'cboTipoDocumento', 'txtFechaEmision'])) {
                if ($("#hfPIDM").val() != "") {
    ...
    var cargaInicial = function () {
        ...
        if (cod != null) {
            ...
            var jqxhr = $.ajax({
                type: "POST",
                url: "vistas/nv/ajax/NVMNDDP.ashx?OPCION=4",
                contentType: false,
                data: data,
                processData: false,
                cache: false,
           ...
           .success(function (datos) {
               ...
               if (datos != null && datos.length > 0) {
                   ...
                   var jqxhr = $.ajax({
                       type: "POST",
                       url: "vistas/nv/ajax/NVMNDDP.ashx?OPCION=5",
                       contentType: false,
                       data: data2,
                       processData: false,
                       async: true
    ...
    var fillTblDocumentosRef = function () {
        $.ajax({
            type: "post",
            url: "vistas/nv/ajax/NVMNDDP.ashx?OPCION=1" +
...
function GrabarNotaDebito() {
    ...
    if (continuar) {
        ...
        var jqxhr = $.ajax({
            type: "POST",
            url: "vistas/nv/ajax/NVMNDDP.ashx?OPCION=3",
            contentType: false,
            data: data,
            processData: false,
            cache: false,
    ...
    var IniciaTabla = function () {
        ...
        $('#tblDatos tbody').on('click', 'tr', function () {
            if ($(this).hasClass('selected')) {
                ...
                var code = row.CODIGO;
                window.location.href = '?f=NVMNDDP&codigo=' + code;
            }
        });
