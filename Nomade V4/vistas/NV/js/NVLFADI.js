var NVLFADI = function () {
    ...
    var crearTablaVacia = function () {
        var parms = {
            ...
            columns: [
                    ...
                    //{
                    //    data: "CODIGO_CLIENTE", createdCell: function (td, cellData, rowData, row, col) { $(td).attr("align", "center"); }
                    //    //width: '30%'
                    ...
                    {
                        data: "TIPO", createdCell: function (td, cellData, rowData, row, col) { $(td).attr("align", "left"); }

                    },
                    {
                        data: "PROD_COD_ANTIGUO", createdCell: function (td, cellData, rowData, row, col) {
                            $(td).attr("align", "center");
                            $(td).html(cellData.split("_N")[0]);
                        }

                    },
                     {
                         data: "PROD_DESC", createdCell: function (td, cellData, rowData, row, col) { $(td).attr("align", "left"); }

                     },
                    {
                        data: "CANTIDAD_VENDIDA", createdCell: function (td, cellData, rowData, row, col) { $(td).attr("align", "right"); }

                    },
                    {
                        data: "UNME_DESC_CORTA", createdCell: function (td, cellData, rowData, row, col) { $(td).attr("align", "center"); },

                    },
                    {
                        data: "DOCUMENTO", createdCell: function (td, cellData, rowData, row, col) {
                            $(td).attr("align", "center");
                            if (rowData.BONIFICACION_IND === 'S') {
                                var lnk = (rowData.VENTA_RAPIDA=='S'?'?f=nvmdovr&codigo=':'?f=nvmdocv&codigo=') + rowData.VTA_CODE ;
                                $($(td).parents("tr")[0])
                                        .css("background-color", "#FFF9B3")
                                        .click(function () {
                                            window.open(lnk, '_blank');
                                        })
                                        .attr("title","Click para ver detalles.");
                    ...
                    {
                        data: "PU", createdCell: function (td, cellData, rowData, row, col) { $(td).attr("align", "right").html(formatoMiles(cellData)); }
                    },
                    {
                        data: "IGV", createdCell: function (td, cellData, rowData, row, col) { $(td).attr("align", "right").html(formatoMiles(cellData)); }
                     },
                    {
                        data: "TOTAL", createdCell: function (td, cellData, rowData, row, col) {$(td).attr("align", "right").html(formatoMiles(cellData)) }
                    },
                    {
                        data: "VEHICULO_PLACA"
                    },
    ...
    var obtenerProdVendidos = function () {
        ...
        $.ajax({
            type: "POST",
            url: "vistas/NV/ajax/NVLPRVE.ashx?OPCION=L2",
            contentType: false,
            data: data,
            dataType: "json",
            processData: false,
    ...
    var eventos = function () {
        ...
        $('#btnMail').click(function (e) {
            ...
            $('#txtAsunto').val("REPORTE DE FACTURACION DIARIA - " + $("#cboEstablecimiento :selected").html());
    ...
    var obtenerBonificaciones = function (cod) {
...
function imprimirTotales() {
    ...
    a.download = "REPORTE_FACTURACION_" + $("#cboEstablecimiento :selected").html() + ".csv";
    a.click();
...
var enviarCorreo = function () {
    ...
    if (vErrors(['cboCorreos', 'txtAsunto'])) {
        ...
        data.append('HTMLMENSAJE', $("#datos_correo").html() +
                                    $("#tblDocumento")[0].outerHTML.replace("table", "table border='1'").replace('style="width: 100%;', 'style="width: 100%;border-collapse:collapse;').split("height:0;").join("").replace("</table>",
