...
var docu = "";
var nroFilasDctos = 1;
var cantItemsSisnot = 0;
var redeondeoTotal = 0;
var prmtCURS = "";
...
var NVMDOCS = function () {
    var nPlazoDiasLinea = '';
    var monto = '';
    var nMontoSaldoDoc;

    var plugins = function () {
        $('#cbo_Empresa, #cbo_Sucursal,#cboVendedor, #cbo_modo_pago, #cboDocumentoVenta, #cboDctoOrigen, #cbo_moneda, #cbo_und_medida,  #cboTipoDoc,#cbo_correlativo,#cboSerieDocVenta').select2();
    ...
    var cargarCorrelativo = function () {
        if ($("#cboDocumentoVenta").val() != "") {
            ...
            AplicarDsctoPorItem(1); //DPORTA SIN-IMPUESTOS
        }
    }
    ...
    var eventoControles = function () {
        ...
        //$("#rbRedondeo").on("change", function () {//Tomando en cuenta el importe total
        ...
        //    var importeTotal = parseFloat($("#txt_subtotal").val());//DPORTA
        //    var importeCobro = parseFloat($("#txt_monto_total").val());//DPORTA
        //    var importeCobrar = parseFloat(gImporteCobrar);
        //    var decTotal = $("#txt_subtotal").val().split('.');
        //    var parteDecT = decTotal[1].substring(1, 2); //decTotal[1];
        //    var decCobrar = $("#txt_monto_total").val().split('.');
        //    var parteDecC = decCobrar[1].substring(1, 2); //decCobrar[1];

        //    if ($(this).is(":checked")) {
        //        if (importeCobro == importeTotal) {
        //            importeTotal += parseFloat($("#txtRedondeo").val());
        //            importeCobrar += parseFloat($("#txtRedondeo").val());
        //        } else if (importeCobro < importeTotal && parteDecC == parteDecT) {//DPORTA
        //            importeTotal += parseFloat($("#txtRedondeo").val());
        //            importeCobrar += parseFloat($("#txtRedondeo").val());
        //        } else if (importeCobro > importeTotal ){
        //            importeTotal += parseFloat($("#txtRedondeo").val());
        //            importeCobrar += parseFloat($("#txtRedondeo").val());
        //        } else {
        //            importeCobrar += parseFloat($("#txtRedondeo").val());
        ...
        $("#btnAgregarDivOrigen").on("click", function () {
            if (vErrors(['cboDctoOrigen'])) {
                if ($("#cboDctoOrigen").val() != "P0027") {
                    if ($("#cboDctoOrigen").val() == "XXXX") {
                        nroFilasDctos++;
                        $("#documentosadd").append("<div class='span12 divDctoOrigenExtra' style='margin-left: 0px'>" +
                            "<div class='span4'></div>" +
                            "<div class='span1'>" +
                            "<div class='control-group'>" +
                            "<label class='span12 control-label'>ID Kardex</label>" +
                            "</div>" +
                            "</div>" +
                            "<div class='span2'>" +
                            "<div class='control-group'>" +
                            "<div class='controls'>" +
                            //"<input id='txtCodKardexOrigen_" + nroFilasDctos + "' class='txtCodKardexOrigen inputOrigenKardex' type='hidden' />" +
                            "<input id='txtKardexDctoOrigen_" + nroFilasDctos + "' class='txtKardexDctoOrigen inputOrigenKardex numeros span12' type='text' disabled style='text-align: center'/>" +
                            "</div>" +
                            "</div>" +
                            "</div>" +
                            "<div class='span1'>" +
        ...
        $("#cboDctoOrigen").on("change", function () {
            ...
            if ($("#cboDctoOrigen").val() == "XXXX") {//SI EL DOCUMENTO DE ORIGEN ES ATENCIONES DE SISNOT
                $("#divFilaCliente2").show();
                $("#txtClienteReferencial").val("");
                $("#NroDocs, #txtDocs, #btnsDocs,  #NroKardex, #txtKardex").show();
            } else {
                $("#divFilaCliente2").hide();
                $("#txtClienteReferencial").val("");
                if ($("#cboDctoOrigen").val() == "") {
                    $("#NroDocs, #txtDocs, #btnsDocs, #NroKardex, #txtKardex").hide();
                } else {
                    $("#NroDocs, #txtDocs, #btnsDocs").show();
                    $("#NroKardex, #txtKardex").hide();
                }
            }
        ...
        $('#rbDescuento').click(function () {//DPORTA 27/08
            var checked = $(this).is(':checked');

            if (checked) {
                $("#txtDescuento").removeAttr("disabled");
                $('#rbAplicaDescuento').removeAttr("disabled");
                $("#txtDescuento").focus();
            } else {
                if ($("#txtDescuento").val() !== '') {
                    RetornaDescuento(parseFloat($("#txtDescuento").val()));
                }
                $("#A1").attr("style", "display:inline");
                $("#txtDescuento").val('');
                $("#txtDescuento").attr("disabled", "disabled");
                $('#rbAplicaDescuento').attr("disabled", "disabled");
                $("#rbAplicaDescuento").prop('checked', false).parent().removeClass('checked');
            }
        });

        $('#rbAplicaDescuento').click(function () {//DPORTA 27/08 
            var checked = $(this).is(':checked');

            if (checked) {
                if ($("#txtDescuento").val() !== '' && parseFloat($("#txt_subtotal").val()) >= parseFloat($("#txtDescuento").val())) {
                    RecalculoImporteConDscto(parseFloat($("#txtDescuento").val()));
                    $("#txtDescuento").attr("disabled", "disabled");
                    $('#rbDescuento').attr("disabled", "disabled");
                    $("#A1").attr("style", "display:none");
                    $('.buscar, .add, .remove').css('display', 'none');//OCULTA LOS BOTONES DE DOC. ORIGEN
                    //$('#rbRedondeo').click().prop('checked', true).parent().addClass('checked');//DPORTA //CON FE Y FUNCIONA 
                } else {
                    if ($("#txtDescuento").val() == '') {
                        infoCustom2("El campo de descuento no puede ser vacío.");
                    } else {
                        infoCustom2("El descuento no puede ser mayor al Importe Total.");
                    }
                    $("#rbAplicaDescuento").prop('checked', false).parent().removeClass('checked');
                    $("#txtDescuento").focus();
                }
                ...
                $("#A1").attr("style", "display:inline");
                $('.buscar, .add, .remove').css('display', 'inline');//MUESTRA LOS BOTONES DE DOC. ORIGEN
                RetornaDescuento(parseFloat($("#txtDescuento").val()));
                $("#txtDescuento").val('');
                $("#txtDescuento,#rbAplicaDescuento").attr("disabled", "disabled");
                $("#rbDescuento").prop('checked', false).parent().removeClass('checked');
                //$('#rbRedondeo').click().prop('checked', false).parent().removeClass('checked');//DPORTA//CON FE Y FUNCIONA 
                //var dsctito = $("#txtRedondeo").val().split('.')//DPORTA//CON FE Y FUNCIONA 
                //if ($("#txtRedondeo").val() != "" && dsctito[1] != "00") {//CON FE Y FUNCIONA 
                //    $('#rbRedondeo').click().prop('checked', true).parent().addClass('checked');//DPORTA
                //}
                $("#rbDescuento").removeAttr("disabled");
            }
        });
        ...
        if (cod !== undefined) {
            ...
            $.ajax({
                ...
                success: function (datos) {
                    ...
                    $("#divFilaCliente2").hide();
                    if ($("#hfCompletoInd").val() == "S") {
                        $("#div_chk_descuento,#div_chk_AplicaDescuento").hide();
                    }
                    ...
                    var kardex = (datos[0].KARDEX).split(",");
                    ...
                    for (var i = 0; i < codigos.length; i++) {
                        ...
                        $("#txtKardexDctoOrigen_" + c + "").val(kardex[i]);
                    }
                    if ($("#hfCompletoInd").val() == "S") {
                        $('.buscar, .add, .remove').css('display', 'none');//OCULTA LOS BOTONES DE DOC. ORIGEN
                    }
                        ...
                        success: function (datos2) {
                            ...
                            if (datos2 != null) {
                                ...
                                for (var i = 0; i < datos2.length; i++) {
                                    ...
                                    //pruebita DPORTA
                                    des_cate_clie = datos2[i].CAT_CODE;
                                    des_cate_clie = datos2[i].CAT_DESC;
                                ...
                                //Pruebita DPORTA
                                $("#hfcod_cate").val(des_cate_clie);
                                $("#hfdes_cate").val(des_cate_clie);
                            }
                    ...
                    //                objProdMuestra.ITEM = datos4[i].ITEM;
...
function RecalculoImporteConDscto(dscto) {//DPORTA 27/08 
    var importeAnticipos = 0;
    if ($("#hfCompletoInd").val() == "N") {
        for (var i = 0; i < detallesVenta.length; i++) {//DPORTA
            if (detallesVenta[i].PROD_CODE_ANTIGUO.substring(0, 2) == "AP") {
                importeAnticipos += detallesVenta[i].PRECIO_DETALLE * -1;
            }
        }
    }
    var importeTotal = parseFloat($("#txt_subtotal").val()) + parseFloat(importeAnticipos);
    var porcentajeDscto = parseFloat((importeTotal - dscto) / importeTotal);
    AplicarDsctoPorItem(porcentajeDscto);
}

function RetornaDescuento(dscto) {//DPORTA 27/08
    var importeAnticipos = 0;
    if ($("#hfCompletoInd").val() == "N") {
        for (var i = 0; i < detallesVenta.length; i++) {//DPORTA
            if (detallesVenta[i].PROD_CODE_ANTIGUO.substring(0, 2) == "AP") {
    ...
    var importeTotal = (parseFloat($("#txt_subtotal").val()) + parseFloat(redeondeoTotal) * -1) + parseFloat(importeAnticipos);
    var porcentajeDscto = parseFloat((importeTotal + dscto) / importeTotal);
    AplicarDsctoPorItem(porcentajeDscto);
}

//RECALCULO DE IMPORTE CON DESCUENTO
function AplicarDsctoPorItem(porcentajeDscto) {//DPORTA 27/08
    if ($("#hfCompletoInd").val() == "N") {
        for (var i = 0; i < detallesVenta.length; i++) {//DPORTA
            if (parseFloat(detallesVenta[i].PRECIO_DETALLE) > 0) {
                ActualizaMontosPorItem(parseFloat(detallesVenta[i].PRECIO_DETALLE * porcentajeDscto), i)
            }
        }
    }
}

function ActualizaMontosPorItem(valor, indice) {//DPORTA
    if ($("#hfCompletoInd").val() == "S") {
        ...
        //if (parseFloat(valor) < parseFloat(precioMinimo) || parseFloat(valor) == "") {
        //    infoCustom2("El valor ingresado no puede ser menor al precio mínimo: " + parseFloat(precioMinimo).toFixed(2))
        //    $(valor).val(parseFloat(precioMinimo).toFixed(2));
        //    $(valor).focus();
        //} else {
...
function CalcularDatosMonetarios2() {
    $(".simboloMoneda").html($("#cbo_moneda :selected").attr("simbolo"));
    ...
    if ($("#hfCompletoInd").val() == "N") {
        ...
        CalcularRedondeoTotal($("#txt_subtotal").val());
        //-------------
        var importeCobrar = parseFloat(gImporteCobrar);
        var importeTotal = parseFloat($("#txt_subtotal").val()) + parseFloat(redeondeoTotal);
        ...
        if ($("#txt_subtotal").val() != 0) {
            $('#rbDescuento').removeAttr("disabled");
        } else {
            $("#rbDescuento, #rbAplicaDescuento").prop("checked", false).parent().removeClass('checked');
            $('#rbDescuento, #rbAplicaDescuento, #txtDescuento').attr("disabled", "disabled");
            $('#txtDescuento').val('');
        }
...
//    Bloquear("ventana")
//    var html =
//        '<div id="_buscarDocumento" style="display: block; width: 90%; left: 5%;"  class="modal hide fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="false" style="display: none;">' +
...
//function setSeleccionDocumento(nro_atencion, cliente, pidm_abogado, moneda) {//POR EL MOMENTO TODOS LOS SERVICIOS ESTÁN EN MONEDA SOLES
...
//                                        objProd.MONTO_DESCUENTO = datos2[i].DESCUENTO;
//                                        objProd.PRECIO_DETALLE = parseFloat(datos2[i].PU).toFixed(prmtDIGP); //El precio está en la misma moneda que  cbo_moneda
//                                        objProd.PU = parseFloat(datos2[i].PU).toFixed(prmtDIGP);
//                                        objProd.TOTAL_BRUTO = (parseFloat(cantidad_producto) * parseFloat(datos2[i].PU)).toFixed(prmtDIGP);
//                                        objProd.TOTAL_NETO = parseFloat(datos2[i].TOTAL).toFixed(prmtDIGP);
//                                        objProd.MONTO_DETRAC = parseFloat(datos2[i].DETRACCION_DECIMALES * datos2[i].TOTAL).toFixed(prmtDIGP);
...
function cargarParametrosSistema() {
    let filtro = "DIGP,REDN,DETR,RETN,RETR,IMRE,BFDV,ADET,ACON,CURS,MIGO,0021,CLIE,GLIT"; //Aquí van los parámetros que se van a utilizar en la pantalla y luego se le hace su case
            ...
            if (datos != null) {
                ...
                if (docu == "") {
                    if (obj.RUC != "") {
                        ...
                        if (obj.CODIGO_TIPO_DOCUMENTO === "6") {
                            $("#hfRUC").val(obj.NRO_DOCUMENTO);
                        }
                        $("#btnHabido").hide();
                    }
                } else {
...
function ValidaPrecioEstandar(venta, min) {
    if (min == "") {
        ...
        var c = parseFloat($("#txtPrecioUnitario").val()).toFixed(2);
        min = 0;
        if ((c.indexOf(".")) > 0) {
            var nroDecimales = c.substring(c.indexOf(".") + 1, c.length).length;
            if (nroDecimales > prmtDIGP) {
    ...
    $("#txtPrecioUnitario").on("blur", function () {
        if (parseFloat($("#txtPrecioUnitario").val()) < parseFloat(min)) {
            ...
            $("#txtPrecioUnitario").val(parseFloat(venta).toFixed(prmtDIGP));
        } else {
            $("#txtPrecioUnitario").val(parseFloat(c).toFixed(prmtDIGP));
        }
    });
    $('#txt_cantidad').keyup();
}
...
function GrabarDctoVenta() {
    ...
    */
    ...
    if ($("#cboDctoOrigen").val() != "") {
        if (vErrors(['cboDocumentoVenta', 'cbo_direccion', 'cboVendedor', 'txtClientes', 'txtNroDctoCliente', 'cbo_Empresa', 'cbo_Sucursal', 'txt_valor_cambio_Oficial', 'txt_valor_cambio', 'txtCodDctoOrigen_1', 'txtSerieDctoOrigen_1', 'txtNroDctoOrigen_1','txtKardexDctoOrigen_1'])) {
            continuar = true;
        } else {
            continuar = false;
        }
    ...
    if (continuar) {
        ...
        if (detallesVenta.length != 0) {
            ...
            if ($("#cboDctoOrigen").val() != "" && $("#cboDctoOrigen").val() != "P0027") {
                ...
                if ($("#cboDctoOrigen").val() == "XXXX") {
                    var kardex = "";
                    var inputs4 = $(".txtKardexDctoOrigen")

                    for (var i = 0; i < inputs4.length; i++) {
                        if (i == inputs4.length - 1) {
                            kardex += $(inputs4[i]).val();
                        } else {
                            kardex += $(inputs4[i]).val() + ",";
                        }
                    }
                }
                data.append('p_KARDEX', kardex);
            ...
            var jqxhr = $.ajax({
                type: "POST",
                url: "vistas/nv/ajax/NVMDOCS.ashx?OPCION=GRABAR_NVMDOCS",
                contentType: false,
                data: data,
                processData: false,
                cache: false
                ...
                .success(function (datos) {
                    if (datos != null) {
                        if (typeof datos[0].CODIGO != "undefined" && datos[0].CODIGO != 'ERROR') {
                            ...
                            } else if (datos[0].CODIGO == 'ERROR_CRE') {
                                alertCustom("Error al registrar datos de crédito. Verifique los datos!");
                            } else if (datos[0].CODIGO == 'ERROR_DTR') {
                                alertCustom("Error al registrar detracción. Verifique los datos!");
                            } else {
                                exito();
                                $("#grabar").html("<i class='icon-pencil'></i> Modificar");
                                $("#grabar").attr("href", "javascript:ActualizarDctoVenta();");
...
function ActualizarDctoVenta() {
    ...
    var continuar = false;
    if ($("#cboDctoOrigen").val() != "") {
        if (vErrors(['cboDocumentoVenta', 'cboVendedor', 'cbo_direccion', 'txtClientes', 'txtNroDctoCliente', 'cbo_Empresa', 'cbo_Sucursal', 'txt_valor_cambio_Oficial', 'txt_valor_cambio', 'txtCodDctoOrigen_1', 'txtSerieDctoOrigen_1', 'txtNroDctoOrigen_1', 'txtKardexDctoOrigen_1'])) {
            continuar = true;
        } else {
            continuar = false;
        }
    ...
    if (continuar) {
        ...
        if (detallesVenta.length != 0) {
            ...
            var jqxhr = $.ajax({
                type: "POST",
                url: "vistas/nv/ajax/NVMDOCS.ashx?OPCION=ACTUALIZAR_NVMDOCS",
                contentType: false,
                data: data,
                processData: false,
                cache: false
                ...
                .success(function (datos) {
                    if (datos != null) {
                        if (typeof datos[0].CODIGO != "undefined" && datos[0].CODIGO != 'ERROR') {
                            ...
                            //noexito();
                            alertCustom("Surgió un error inesperado al modificar la venta. Intente nuevamente, por favor.");
                        }
                    } else {
                        noexito();
                    }
...
function CompletarDctoVenta() {
    ...
    if ($("#cboDctoOrigen").val() != "") {
        if (vErrors(['cboDocumentoVenta', 'cboVendedor', 'cbo_direccion', 'txtClientes', 'cboDocumentoVenta', 'cbo_Empresa', 'txtNroDctoCliente', 'cbo_Sucursal', 'cboSerieDocVenta', 'txtNroDocVenta', 'txt_valor_cambio_Oficial', 'txt_valor_cambio', 'txtCodDctoOrigen_1', 'txtSerieDctoOrigen_1', 'txtNroDctoOrigen_1', 'txtKardexDctoOrigen_1'])) {
            continuar = true;
        } else {
            continuar = false;
        }
    ...
    if (continuar) {
        ...
        if (detallesVenta.length != 0) {
            ...
            var jqxhr = $.ajax({
                type: "POST",
                url: "vistas/nv/ajax/NVMDOCS.ashx?OPCION=COMPLETAR_NVMDOCS",
                contentType: false,
                data: data,
                processData: false,
                cache: false
...
function BuscarDocumentoOrigen(btn) {
    if ($("#cboDctoOrigen").val() == "XXXX") { //SI DOCUMENTO DE ORIGEN ES ATENCIONES
        if (vErrors(['cboDctoOrigen', 'cbo_Sucursal'])) {
            ...
            var inputKardex = $($(btn).parent().parent().parent().parent().find(".inputOrigenKardex")[0])

            var cod = $(inputCod).val()

            var serie = $(inputSerie).val()
            var nro = $(inputNro).val()
            var kardex = $(inputKardex).val()

            if (cod == "") {
                $('#tblDocumentosOS').DataTable().destroy();
                var tbody = $('#tblDocumentosOS').find('tbody');
                $.ajax({
                    type: 'post',
                    url: 'vistas/nv/ajax/nvmdocs.ashx?OPCION=LDOCS' +
                    ...
                    success: function (datos) {
                        ...
                        if (datos != null) {
                            for (var i = 0; i < datos.length; i++) {
                                ...
                                tbody.append('<tr><td style="text-align: center">' + datos[i].NRO_DOCUMENTO + '</td><td style="text-align: center">' + datos[i].KARDEX + '</td><td style="text-align: center">' + datos[i].CLIENTE + '</td><td style="text-align: center">' + datos[i].VALOR_TOTAL_OS + '</td><td style="text-align: center">' + datos[i].NOMBRE_ABOGADO + '</td><td style="text-align: center">' + datos[i].FECHA + '</td><td style="text-align: center">' + datos[i].PIDM_ABOGADO + '</td></tr>');
                            }
                        }
                    },
                    error: function (msg) {
                        ...
                        if (typeof cod_doc != "undefined") {
                            ...
                            if ($("#txtCodDctoOrigen_1").val() != cod_doc) {
                                ...
                                if (continuar) {
                                    ...
                                    inputNro.val(nro);
                                    inputKardex.val(kardex);
                                    $('#divBuscarDocOS').modal('hide');
                                    $('#txtClienteReferencial').val(clinteRef);
                                    $("#cboVendedor").select2("val", pidm_abogado).change(); //AQUÍ IRAN LOS ABOGADOS 
                                    if ($("#cboVendedor").val() != '') {
                                        $("#cboVendedor").attr("disabled", "disabled");
                                    } else {
                                    ...
                                    //if ($('#txtClienteReferencial').val() != "CLIENTES VARIOS") { PARA QUE CARGUE POR DEFECTO CLIENTES VARIOS SI SELECCIONA CLIENTES DIFERENTES
                                    //    if ($('#txtClienteReferencial').val() != "" && $('#txtClienteReferencial').val() != clinteRef) {
                                    //        $('#txtClienteReferencial').val("CLIENTES VARIOS");
                                    //        cargarprederteminado = true;
                                    //        fillTxtCliente("#txtClientes", '1');
                                    //        console.log(jsonPredeterminado.PIDM);
                                    //    } else {
                                    //        $('#txtClienteReferencial').val(clinteRef);
                                    //    }
                ...
                if (cod == "") {
                    ...
                    $.ajax({
                        ...
                        success: function (datos) {
                            ...
                            if (datos != null) {
                                for (var i = 0; i < datos.length; i++) {
                                    tbody.append('<tr><td style="text-align: center">' + datos[i].CODIGO + '</td><td style="text-align: center">' + datos[i].NRO_DOCUMENTO + '</td><td style="text-align: center">' + datos[i].CLIENTE + '</td><td style="text-align: center">' + datos[i].FECHA + '</td></tr>');
                                }
                            }
...
var NuevaVenta = function () {
    window.location.href = '?f=NVMDOCS'
    //Bloquear("ventana");
    //$("#lblContraentrega").hide();
...
function CargarDatosDocumentoOrigen(inputCod, btn) {
    ...
    $.ajax({
        type: "POST",
        url: "vistas/nv/ajax/nvmdocs.ashx?OPCION=LDOCSD",
        contentType: false,
        data: data,
        processData: false,
        cache: false
            ...
            if ($("#cboDctoOrigen").val() == "XXXX") {
                        ...
                        $.ajax({
                            ...
                            success: function (datos2) {
                                if (datos2 != null) {
                                    for (var i = 0; i < datos2.length; i++) {
                                        var objProd = {};
                                        contItem = detallesVenta.length;
                                        var item = contVenta + 1;
                                        objProd.ITEM_TABLA = item;
                                        objProd.ITEM = contItem + 1; //item_producto
                                        objProd.CODIGO = codigo_producto;
                                        objProd.PROD_CODE_ANTIGUO = datos2[i].PROD_CODIGO_ANTIGUO;
                                        objProd.TIPO_BIEN = datos2[i].TIPO_BIEN;
                                        ...
                                        if (tipoDocCode == '0001' || tipoDocCode == '0003' || tipoDocCode == '0012') {//DPORTA SIN-IMPUESTOS
                                            if ($("#cbo_moneda").val() == '0002') {//SERVICIOS EN SOLES
                                                //EL PRECIO VIENE DIRECTAMENTE DE SISNOT
                                                objProd.MONTO_DESCUENTO = datos2[i].DESCUENTO;
                                        ...
                                        objProd.COSTO_PRODUCTO = 0;
                                        objProd.ALMACENABLE = "N";
                                        objProd.CODE_DCTO_ORIGEN = inputCod.val();
                                    ...
                                    if (nroDetallesInsertados == cantItemsSisnot && nroDetallesInsertados > 0) {

                                        if (nroDetallesInsertados == cantItemsSisnot) {
                                            exitoCustom("Todos los detalles del documento de origen se cargaron correctamente.");
                                        }
                                        else {
                                            exitoCustom("(" + nroDetallesInsertados + " de " + cantItemsSisnot + ") detalles del documento de origen se cargaron correctamente.");
                                        }
                                    }
                                }
                            },
                ...
                if (datos != null && datos != "") {
                    for (var i = 0; i < datos.length; i++) {
                        ...
                        var cantidad = $("#txt_cantidad").val();
                        var unidadMedidaCode = datos[i].UNIDAD;
                        var unidadMedida = datos[i].DESC_UNIDAD;
...
function CargarOrdenCompraCliente() {
    ...
    $.ajax({
        ...
        success: function (datos) {
            ...
            CalcularRedondeoTotal(datos[0].VALOR);
...
function EliminarDatosDocumentoOrigen(btn, first) {
    ...
    if (first != undefined && first == "first") { // Eliminar el Primer Dcto        
        ...
        $("#txtCodDctoOrigen_1,#txtSerieDctoOrigen_1,#txtNroDctoOrigen_1,#txtKardexDctoOrigen_1").val("");
...
function CalcularRedondeoTotal(importeTotal) {//Param en string

    var parametroRedondeo = $("#hfParamRedondeo").val();
    importeAux = parseFloat(importeTotal).toFixed(2).toLocaleString();
    var strRedondeo;
    var centecima = importeAux.substring(importeAux.length - 1, importeAux.length)
    if (parametroRedondeo == "ARR") {
        var diferencia = 10 - parseFloat(centecima);
    ...
    //}
    //if ($("#rbRedondeo").is(":checked")) {
    //    importeTotal += parseFloat(redondeo);
    //}

    redeondeoTotal = redondeo; //$("#txtRedondeo").val(redondeo);
    //$("#rbRedondeo").is(":checked") ? $("#txtRedondeo2").val(redondeo) : $("#txtRedondeo2").val("0.00");
    //$("#txtDonacion").val(donacion);
    //$("#rbDonacion").is(":checked") ? $("#txtDonacion2").val(donacion) : $("#txtDonacion2").val("0.00");
}
