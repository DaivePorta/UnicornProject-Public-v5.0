var NVLPREV = function () {
    ...
    var crearTablaVacia = function () {
        var parms = {
            ...
            columns: [
                    ...
                    {
                        data: "PROD_COD_ANTIGUO", createdCell: function (td, cellData, rowData, row, col) { $(td).attr("align", "left"); }
                    ...
                    {
                        data: "UNME_CORTO", createdCell: function (td, cellData, rowData, row, col) { $(td).attr("align", "center"); },
    ...
    var obtenerProdVendidos = function () {
        ...
        $.ajax({
            type: "POST",
            url: "vistas/NV/ajax/NVLPRVE.ashx?OPCION=L3",
            contentType: false,
            data: data,
            dataType: "json",
            processData: false,
    ...
    var eventos = function () {
        ...
        $('#btnMail').click(function (e) {
            ...
            $('#txtAsunto').val("REPORTE PREVENTA - " + $("#cboEstablecimiento :selected").html());
...
var enviarCorreo = function () {
    ...
    if (vErrors(['cboCorreos', 'txtAsunto'])) {
        ...
        data.append('HTMLMENSAJE', $("#datos_correo").html() +
                                    $("#tblDocumento")[0].outerHTML.replace("table", "table border='1'").replace('style="width: 100%;', 'style="width: 100%;border-collapse:collapse;')
                                    .split("height:0;").join("")
                                   );           
