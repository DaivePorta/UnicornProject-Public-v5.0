    ...
    var eventoControles = function () {
        $('#btnFiltrar').click(function () {
            if (vErrors(['cboEmpresa', 'cboMon', 'cboAnios'])) {
                ...
                $.ajax({
                    type: "post",
                    url: 'vistas/NV/ajax/NVLRAVM.ashx?OPCION=REPORTE_ANUALES&CTLG_CODE=' + $('#cboEmpresa').val() + '&SCSL_CODE=' + $('#cboSucursal').val() + '&MONE_CODE=' + $('#cboMon').val() + '&ANIO=' + $('#cboAnios').val().toString(),
                    contenttype: "application/json",
                    datatype: "json",
                    success: function (datos) {
                        if (datos !== null) {
                        ...
                        $('#chart').highcharts({
                            ...
                            title: { text: 'Analítico de ventas anuales' },
                            legend: {
                                enabled: false
                            },
                            yAxis: {
                        ...
                        $('#pie').highcharts({
                            ...
                            title: { text: 'Torta Analítico de ventas anuales' },
                            tooltip: {
                                formatter: function () {
                                    return '<b>' + this.point.x + '</b><br/>' +
                                         $('#cboMon :selected').attr('data-simbolo') + ' ' + formatoMiles(this.point.y);
