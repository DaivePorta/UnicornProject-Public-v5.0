var NVLRAMV = function () {
    ...
    var eventoControles = function () {
        ...
        $('#btnFiltrar').click(function () {
            if (vErrors(['cboEmpresa', 'cboMon', 'txtAnio'])) {
                ...
                $.ajax({
                    type: "post",
                    url: 'vistas/NV/ajax/NVLRAVM.ashx?OPCION=MARCAS_VENDIDAS&CTLG_CODE=' + $('#cboEmpresa').val() + '&SCSL_CODE=' + $('#cboSucursal').val() + '&MONE_CODE=' + $('#cboMon').val() + '&ANIO=' + $('#txtAnio').val(),
                    contenttype: "application/json",
                    datatype: "json",
                    success: function (datos) {
                        if (datos !== null) {
                            ...
                            for (var i = 0 ; i < datos.length; i++) {
                                $('#tblReporteMostrar tbody').append('<tr><td style="text-align: center">' + ((i + 1) === datos.length ? 0 : (i + 1)) + '</td><td style="display: none">' + datos[i].COD_MARCA + '</td>\
                                <td>' + datos[i].MARCA + '</td>\
                                ...
                                $('#tblReporte tbody').append('<tr><td>' + datos[i].MARCA + '</td><td>' + parseFloat(datos[i].MONTO).toFixed(2) + '</td></tr>');
                        ...
                        $('#pie').highcharts({
                            ...
                            title: { text: 'Analítico de Marcas vendidas' },
                            tooltip: {
                                formatter: function () {
    ...
    var fillBandeja = function () {
        ...
        oTableVentas = iniciaTabla('tbl_ventas_marcas', parms);
        $('#tbl_ventas_marcas').removeAttr('style');

        $('#tbl_ventas_marcas tbody').on('dblclick', 'tr', function () {
    ...
    var funcionalidadTabla = function () {
        ...
        $('#tblReporteMostrar tbody').on('click', 'tr', function () {
            ...
            var COD_MARCA = $($(this).find('td')[1]).text();
            var EMPRESA = $('#cboEmpresa').val();
            var ESTABLECIMIENTO = $('#cboSucursal').val();
            var MONEDA = $('#cboMon').val();
            var ANIO = $('#txtAnio').val();

            var MARCA = $($(this).find('td')[2]).text();

            console.log(COD_MARCA);

            if (COD_MARCA == '0' || COD_MARCA == '') {
                infoCustom2("No se encontraron datos!");
                return;
            }

            $("#labelReporteDetalle").text('Detalle de Ventas por la marca:' + MARCA);
            Bloquear("ventana");
            $.ajax({
                type: "post",
                url: 'vistas/NV/ajax/NVLDOCT.ashx?OPCION=6&CTLG_CODE=' + EMPRESA + '&SCSL_CODE=' + ESTABLECIMIENTO + '&MONEDA_CODE=' + MONEDA + '&ANIO=' + ANIO + '&CODE_MARCA=' + COD_MARCA,
                contenttype: "application/json;",
                datatype: "json",
                async: true,
                success: function (datos) {
