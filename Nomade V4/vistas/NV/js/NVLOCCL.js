var NVLOCCL = function () {
    ...
    var plugins = function () {
        ...
        /* inicializacion de fechas pára el reporte*/
        $('#txtHasta').datepicker("setDate", new Date());
        let fechahoy = new Date();
        $("#txtDesde").datepicker("setDate", '01-' + (fechahoy.getMonth() + 1).toString() + '-' + fechahoy.getFullYear().toString());
    ...
    var obtenerDocumentos = function () {
        ...
        var jqxhr = $.ajax({
            type: "POST",
            url: "vistas/NV/ajax/NVLOCCL.ashx?OPCION=3",
            contentType: false,
            data: data,
            processData: false,
            cache: false
                ...
                if (datos != null) {
                    ...
                    $('#tblDocumento tbody').on('click', 'tr', function () {
                        if ($(this).hasClass('selected')) {
                            ...
                            var code = row[0];
                            window.location.href = '?f=nvmoccl&codigo=' + code;
                        }
                    });
...
function imprimirListaDctosVenta() {
    ...
    var jqxhr = $.ajax({
        type: "POST",
        url: "vistas/NV/ajax/NVLOCCL.ashx?OPCION=5",
        async: false,
        contentType: false,
        data: data,
        processData: false,
        ...
        success: function (datos) {
            ...
            if (datos != null) {
                ...
                $("#divDctoImprimir").prepend("<h5 class='arial'>ORDENES DE COMPRA CLIENTE - " + nomSucursal + "</h5>")
