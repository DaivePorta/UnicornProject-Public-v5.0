<%@ WebHandler Language="VB" Class="NVLRASV" %>

Imports System
Imports System.Web
Imports System.Data

Public Class NVLRASV : Implements IHttpHandler
    Dim OPCION As String
    Dim p_CTLG_CODE, p_SCSL_CODE, p_MONE_CODE, p_ANIO As String
        ...
        Try
            Select Case OPCION
                Case "1" 'Generar tabla 
                    ...
                    dt = nvVenta.ReporteAnaliticoSubgruposVendidos(p_CTLG_CODE, p_SCSL_CODE, p_MONE_CODE, p_ANIO)
    ...
    Public Function GenerarTablaDatos(ByVal dt As DataTable) As String
        ...
        resb.AppendFormat("<th style='text-align:center;'>SUBGRUPO</th>")
        ...
        If Not (dt Is Nothing) Then
            For i As Integer = 0 To dt.Rows.Count - 1
                resb.AppendFormat("<tr>")
                If dt.Rows(i)("CODIGO_SUBGRUPO").ToString() <> "TOTA" Then
                    resb.AppendFormat("<td style='text-align:center;'>{0}</td>", dt.Rows(i)("NOMBRE_SUBGRUPO").ToString())
        ...
        resb.AppendFormat("<th style='text-align:left; display:none;'>COD_SUBGRUPO</th>")
        resb.AppendFormat("<th style='text-align:left;'>SUBGRUPO</th>")
        ...
        If Not (dt Is Nothing) Then
            For i As Integer = 0 To dt.Rows.Count - 1
                ...
                If dt.Rows(i)("CODIGO_SUBGRUPO").ToString() = "TOTA" Then
                    ...
                    resb.AppendFormat("<td style='text-align:left; display:none'>{0}</td>", dt.Rows(i)("CODIGO_SUBGRUPO").ToString())
                    resb.AppendFormat("<th style='text-align:left;'>{0}</th>", dt.Rows(i)("NOMBRE_SUBGRUPO").ToString())
                ...
                Else
                    If dt.Rows(i)("CODIGO_SUBGRUPO").ToString() = "OTRO" Then
                        resb.AppendFormat("<td style='text-align:center;'>0</td>")
                    Else
                        resb.AppendFormat("<td style='text-align:center;'>{0}</td>", i + 1)
                    End If
                    resb.AppendFormat("<td style='text-align:left; display:none'>{0}</td>", dt.Rows(i)("CODIGO_SUBGRUPO").ToString())
                    resb.AppendFormat("<td style='text-align:left;'>{0}</td>", dt.Rows(i)("NOMBRE_SUBGRUPO").ToString())
