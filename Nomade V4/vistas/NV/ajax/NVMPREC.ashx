...
Public Class NVMPREC : Implements IHttpHandler
        ...
        Try
            Select Case OPCION
                Case "1" 'Listar PRODUCTOS Y PRECIOS
                    ...
                    res = GenerarTablaMantenimientoPrecios(dtRangos, dtProds)

                Case "2" 'Registrar Precio cantidad
                    context.Response.ContentType = "application/text; charset=utf-8"
                    'nmGestionPrecios.QUITAR_PRECIO_CANTIDAD(PROD_CODE, CTLG, RANG_CODE, ALMC_CODE)
                    res = nmGestionPrecios.REGISTRAR_PRECIO_CANTIDAD(PROD_CODE, CTLG, ALMC_CODE, RANG_CODE, PLAN_CODE, MONE_CODE,
                ...
                Case "3" 'Registrar Precio cantidad para Todos los ALMACENES
                    ...
                    Try
                        ...
                        For Each MiDataRow As DataRow In dt.Rows
                            res = nmGestionPrecios.REGISTRAR_PRECIO_CANTIDAD(PROD_CODE, CTLG, MiDataRow("CODIGO"), RANG_CODE, PLAN_CODE, MONE_CODE,
                                                                             PRECIO_VENTA, PRECIO_VENTA, UTILIDAD, FTVPRER_CODE, USUA_ID)
                        Next
                ...
                Case "5" 'Generar tabla de precios para impresion
                    ...
                    context.Response.ContentType = "application/text; charset=utf-8"
                    dtRangos = nmGestionPrecios.ListarRangoPrecioCantidad("")
                    dtProds = nmGestionPrecios.ListarProductoPrecios(IIf(PROD_CODE Is Nothing, "", PROD_CODE), CTLG, SCSL, "C", IIf(PRECIO_FILTRO Is Nothing, "", PRECIO_FILTRO),
                                                                     IIf(GRUP_CODE Is Nothing, "", GRUP_CODE), IIf(SUBGRUP_CODE Is Nothing, "", SUBGRUP_CODE), "", p_COSTO_IND,
                                                                     IIf(p_PROD_SIN_STOCK_IND Is Nothing, "S", p_PROD_SIN_STOCK_IND),
                                                                     IIf(p_MARC_CODE Is Nothing, "", p_MARC_CODE))
                    res = GenerarTablaImprimir(dtRangos, dtProds)
    ...
    Public Function GenerarTablaMantenimientoPrecios(ByVal dtRangos As DataTable, ByVal dtProds As DataTable) As String
