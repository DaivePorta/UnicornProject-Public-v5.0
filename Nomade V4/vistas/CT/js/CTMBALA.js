...
var CTMBALA = function () {

    var plugins = function () {
        $('#cboEmpresa').select2();       
        $('#cboEstablecimiento').select2(); 
        $('#cboAcreedorDeudor').select2();
        $('#cboRealizacion').select2();
        $('#txtDesde').datepicker();
        $('#txtHasta').datepicker();
    ...
    var listarFacturasCompra = function () {
        if (vErrors(["cboEmpresa"])) {
            ...
            data.append('p_REALIZACION', ($("#cboRealizacion").val() === null ? '' : $("#cboRealizacion").val().toString()));
            data.append('p_ACREEDOR_DEUDOR', $("#cboAcreedorDeudor").val());

                 
            Bloquear("ventana");
            var jqxhr = $.ajax({
                type: "POST",
                url: "vistas/CT/ajax/CTMBALA.ashx?OPCION=3",
                contentType: false,
                data: data,
                processData: false,
                cache: false
           ...
           .success(function (datos) {
               ...
               if (datos != null) {
                   ...
                   $("#lblSimboloMonedaAlterna,#lblSimboloMonedaAlterna0").html($("#hfSimbMonedaAlterna").val());
...
$(function () {
    ...
    $('#tblBandeja tbody td img').live('click', function () {
        ...
        var p_REALIZACION = ($("#cboRealizacion").val() === null ? '' : $("#cboRealizacion").val().toString());
        var p_ACREEDOR_DEUDOR = $("#cboAcreedorDeudor").val();
        ...
        if (oTable.fnIsOpen(nTr)) {
            ...
            $.ajax({
                type: "POST",
                url: "vistas/CT/ajax/CTMBALA.ashx?OPCION=4&p_FACTURA=" + id + "&p_CTLG=" + ctlg + '&p_SCSL_CODE=' + p_SCSL_CODE
                + "&p_DESDE=" + p_DESDE + "&p_HASTA=" + p_HASTA + "&p_ACREEDOR_DEUDOR=" + p_ACREEDOR_DEUDOR + "&p_REALIZACION=" + p_REALIZACION,
                async: true,
                success: function (datos) {                  
                    $('#c' + id).html(datos);
