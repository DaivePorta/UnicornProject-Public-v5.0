<%@ WebHandler Language="VB" Class="CTMTIAS" %>

Imports System
Imports System.Web
Imports System.Data

Public Class CTMTIAS : Implements IHttpHandler
    Dim OPCION As String
    Dim CODE, MARCA, ESTADO_IND, USUA_ID As String
    Dim NOMBRE, ORDEN, DESCRIPCION, MNEMO As String
    Dim CTLG_CODE, p_CTLG_CODE As String
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        ORDEN = context.Request("ORDEN")
        DESCRIPCION = context.Request("DESCRIPCION")
        MNEMO = context.Request("MNEMO")
        p_CTLG_CODE = context.Request("p_CTLG_CODE")

        Select Case OPCION
            Case "1"
                Dim m As New Nomade.NM.NMGestionDeMarcas("Bn")
                res = m.CambiarEstadoGestionAsientos(CODE)
                m = Nothing
            Case "CM"
                Dim m As New Nomade.NM.NMGestionDeMarcas("Bn")
                res = m.CrearGestionAsientos(NOMBRE, DESCRIPCION, ESTADO_IND, ORDEN, USUA_ID)
                m = Nothing
            Case "AM"
                Dim m As New Nomade.NM.NMGestionDeMarcas("Bn")
                res = m.ActualizarGestionAsientos(CODE, NOMBRE, DESCRIPCION, ESTADO_IND, ORDEN, USUA_ID)
                m = Nothing
            Case "CMS"
                Dim m As New Nomade.NM.NMGestionDeMarcas("Bn")
                res = m.CrearGestionAsientosDetalle(CTLG_CODE, MNEMO, CODE)
                m = Nothing
            Case "RM" 'LISTAR UNA MARCA EN ESPECIFICO

                context.Response.ContentType = "application/json; charset=utf-8"
                Dim m As New Nomade.NM.NMGestionDeMarcas("Bn")
                dt = m.ListarGestionAsientos(CODE, String.Empty, CTLG_CODE)
                If Not (dt Is Nothing) Then
                    ...
                    resb.Append("""NRO_ORDEN"" :" & """" & dt.Rows(0)("NRO_ORDEN") & """,")
            ...
            Case "69"
                context.Response.ContentType = "application/json; charset=utf-8"
                Dim rf As New Nomade.NM.NMGestionDeMarcas("Bn")
                dt = rf.ListarOperaciones(p_CTLG_CODE, "A")
                If Not (dt Is Nothing) Then
                    resb.Append("[")
                    For Each row As DataRow In dt.Rows
                        resb.Append("{")
                        resb.Append("""CODIGO"" :" & """" & row("CODIGO") & """,")
                        resb.Append("""NOMBRE"" :" & """" & row("NOMBRE") & """,")
            ...
            Case "CEMS"
                Dim m As New Nomade.NM.NMGestionDeMarcas("Bn")
                res = m.CambiarEstadoGestionAsientos(MNEMO, CODE, CTLG_CODE)
                m = Nothing
            Case "2"
                context.Response.ContentType = "application/json; charset=utf-8"
