var NALLRIF = function () {
        ...
        $.ajax({
            ...
            success: function (datos) {
                ...
                if (datos != null) {
                    ...
                    llenarRuc();
                    listarAlmacenes($('#ctl00_hddctlg').val());
                    $("#txtDescr").remove();
    ...
    var eventoComtroles = function () {
        ...
        $('#buscar').on('click', function () {
            if (vErrors(["txtTipo", "txtExist", "txtTipo", "txtCodUni", "txtRuc", "txtanio", "cboMes"])) {
                listar();
            }
        });
    ...
    function Exportar() {
        ...
        $.ajax({
            type: "POST",
            url: "vistas/na/ajax/NALLRIF.ashx?OPCION=7",
            //&p_anio=" + $('#txtanio option:selected').html() + "&p_mes=" + $('#cboMes option:selected').html() + "&p_ruc=" + $('#txtRuc').val() + "&p_emp=" + $('#cboEmpresas option:selected').html() + "&p_direcc=" + $('#txtDireccion').val() + "&p_codExis=" + $('#txtExist').val() + "&p_tipoExist=" + $('#txtTipo').val() + "&p_Descr=" + $('#txtDescr').val() + "&p_unidad=" + $('#txtCodUni').val() + "&p_alamcen1=" + $('#hf10').val() +  "&p_scl1=" + $('#cboEmpresas').val() + "&p_prd1=" + $('#hdcodProd2').val(),
            //  url: "vistas/na/ajax/nalmerc.ashx?OPCION=4&p_alamcen=" + $('#hf10').val() + "&p_grupo=" + grupos + "&p_scl=" + empresa + "&p_UNME_DET=" + unmed + "&p_TIPO=" + tipo,
            contentType: false,
            data: data,
    ...
    function listar() {
        ...
        $.ajax({
            type: "POST",
            url: "vistas/na/ajax/NALLRIF.ashx?OPCION=8&p_alamcen=" + $('#hf10').val() + "&p_scl=" + $('#cboEmpresas').val() + "&p_prd=" + $('#hdcodProd2').val() + "&p_anio=" + $('#txtanio').val() + "&p_mes=" + $('#cboMes').val(),
            async: false,
            success: function (datos) {
                Desbloquear("ventana");
                if (datos != null) {
