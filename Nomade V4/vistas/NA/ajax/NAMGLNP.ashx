<%@ WebHandler Language="VB" Class="NAMGLNP" %>

Imports System
Imports System.Web
Imports System.Data

Public Class NAMGLNP : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        Select Case opcion
            Case "1"
                ...
                dt = p.fListarLineaCredito(proveedor, empresa)
            ...
            Case "A"
                res = p.fActualizarEstado(proveedor, secuencia, empresa)
            Case "2"
                context.Response.ContentType = "application/json; charset=utf-8"
                dt = p.fObtenerSEQ(proveedor, empresa)
                If Not (dt Is Nothing) Then
                    resb.Append("[")
                    For Each MiDataRow As DataRow In dt.Rows
                        resb.Append("{")
            ...
            Case "4"
                context.Response.ContentType = "application/json; charset=utf-8"
                dt = p.fListarLCREDPROVXEMP(empresa)
                If Not dt Is Nothing Then
                    resb.Append("[")
                    For Each MiDataRow As DataRow In dt.Rows
                        resb.Append("{")
