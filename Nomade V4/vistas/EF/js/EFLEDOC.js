...
var EFLEDOC = function () {
    ...
    var crearTablaVacia = function () {
                    ...
                    data: null, createdCell: function (td, cellData, rowData, row, col) {
                        ...
                        if (rowData.ANULADO_IND === 'S' && rowData.ESTADO_ELECT !== "DE BAJA") {
                            if (rowData.ESTADO_ELECT !== "ENVIADO") {
                                buttons += `<button type="button" class='btn btnTbl black' action="enviar2" data-toggle="tooltip" data-placement="bottom" title="De Baja"><i class='fa fa-thumbs-down'></button>`;

                            }
                        }
                        if ((rowData.ESTADO_ELECT == 'NO ENVIADO' || rowData.ESTADO_ELECT == 'ERROR') && rowData.ANULADO_IND === 'N') {
                            //buttons += `<button type="button" class='btn btnTbl blue' action="enviar2" data-toggle="tooltip" data-placement="bottom" title="Esperando Respuesta"><i class='fa fa-clock-o'></button>`;
                            buttons += `<button type="button" class='btn btnTbl green' action="descargar" data-toggle="tooltip" data-placement="bottom" title="Descargar"><i class='fa fa-download'></button>`;
                        } else {
                            if (rowData.ESTADO_ELECT == 'ENVIADO') {

                                if (rowData.ANULADO_IND === 'S') {
                                    buttons += `<button type="button" class='btn btnTbl black' action="verificar_anulacion2" data-toggle="tooltip" data-placement="bottom" title="Anulado"><i class='fa fa-thumbs-down'></button>`;
                                ...
                                if (rowData.ESTADO_ELECT == 'PENDIENTE') {
                                    ...
                                    if (rowData.ESTADO_ELECT == 'PENDIENTE DE BAJA') {
                                        buttons += `<button type="button" class='btn btnTbl red' action="verificar_baja2" data-toggle="tooltip" data-placement="bottom" title="De Baja"><i class='fa fa-clock-o'></button>`;
        ...
        $('#tblDocumento tbody').on('click', 'button', function () {
            ...
            codigoParaPDF = row.COD_DOCUMENTO;
            let ruc = row.RUC;
            let cod_sunat = row.COD_SUNAT;
            let serie = row.SERIE;
            let correlativo = row.CORRELATIVO;

            $("#ctl00_cph_ctl00_PCONGEN1_ctl00_hddCodVenta").val(codigoParaPDF);
            $("#ctl00_cph_ctl00_PCONGEN1_ctl00_hddRuc").val(ruc);
            $("#ctl00_cph_ctl00_PCONGEN1_ctl00_hddCodDoc").val(cod_sunat);
            $("#ctl00_cph_ctl00_PCONGEN1_ctl00_hddSerie").val(serie);
            $("#ctl00_cph_ctl00_PCONGEN1_ctl00_hddNumDoc").val(correlativo);
            $("#ctl00_cph_ctl00_PCONGEN1_ctl00_hddCtlg").val($("#cboEmpresa").val()); //DPORTA 11/03/2022
            console.log(row);
            ...
            switch ($(this).attr("action")) {
                ...
                case "descargar":
                    ...
                    break;
                case "enviar2":
                    infoCustom2("El documento " + row.DOCUMENTO + " aún no ha sido procesado por SUNAT");
                    break;
                case "verificar_anulacion2":
                    alertCustom("El documento " + row.DOCUMENTO + " ha sido anulado en el sistema");
                    break;
                case "verificar_baja2":
                    infoCustom("El documento " + row.DOCUMENTO + " ha sido dado de baja por SUNAT");
                    break;                
            }
    ...
    //        var sRuta = "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=" + sOpcion + "&sCodEmpresa=" + sCodEmpresa + "&sCodVenta=" + sCodVenta;
    //        $.ajax({
    //            type: "post",
    //            url: sRuta,
    //            contenttype: "application/json;",
    ...
    //            var sCodVenta = documento;
    //            let sRuta = "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=VNCORB&sCodEmpresa=" + sCodEmpresa + "&sCodNC=" + sCodVenta
    ...
    //                var sOpcion = 'FACT';
    //                var sCodVenta = documento;
    //                let sRuta = "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=VND&sCodEmpresa=" + sCodEmpresa + "&sCodND=" + sCodVenta
    ...
    //        if (sCodTipoDoc == '0007') {   // NOTA DE CREDITO                
    //            var sCodVenta = documento;
    //            let sRuta = "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=VBAJANC&sCodEmpresa=" + sCodEmpresa + "&sCodNC=" + sCodVenta
    ...
    //                var sOpcion = 'FACT';
    //                var sCodVenta = documento;
    //                let sRuta = "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=VBAJAND&sCodEmpresa=" + sCodEmpresa + "&sCodND=" + sCodVenta
    ...
    //        if (sCodTipoDoc == '0007') {   // NOTA DE CREDITO                
    //            var sCodVenta = documento;
    //            let sRuta = "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=NC_ORBI&sCodEmpresa=" + sCodEmpresa + "&sCodNC=" + sCodVenta
    ...
    //                var sOpcion = 'FACT';
    //                var sCodVenta = documento;
    //                let sRuta = "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=ND&sCodEmpresa=" + sCodEmpresa + "&sCodND=" + sCodVenta
    ...
    //    var sCodVenta = documento;
    //    var sRuta = "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=MEFACT&sCodVenta=" + sCodVenta + "&sTipoDoc=" + $("#cboDocumento").val();
    //    $.ajax({
    //        type: "post",
    //        url: sRuta,
    //        contentType: false,
    ...
    //        if (sCodTipoDoc == '0007') {   // NOTA DE CREDITO                
    //            var sCodVenta = documento;
    //            let sRuta = "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=BNC&sCodEmpresa=" + sCodEmpresa + "&sCodNC=" + sCodVenta
    ...
    //                var sOpcion = 'FACT';
    //                var sCodVenta = documento;
    //                let sRuta = "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=BND&sCodEmpresa=" + sCodEmpresa + "&sCodND=" + sCodVenta
    ...
    //    Bloquear("ventana");
    //    let sRuta = "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=TEST_CONEC_FTP_EFACT"
    ...
    var eventoControles = function () {
        ...
        $('#btnFiltrarFactElec').on('click', function () {
            ...
            if (vErrors(dataFields)) {
                ...
                $.ajax({
                    type: "post",
                    url: "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=BLFE&sCodEmpresa=" + emp + "&sSucursal=" + suc + "&sTipoDoc=" + doc + "&sCliente=" + cli + "&sEmision=" + emi + "&sDesde=" + des + "&sHasta=" + has,
                    contenttype: "application/json;",
                    datatype: "json",
                    async: true,
    ...
    //        url: "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=SUBIR_EFACT&sRuta=" + sRuta,
    //        success: function (datos) {
    ...
    //        url: "vistas/EF/ajax/EFLEDOC.ashx?sOpcion=BAJAR_EFACT&sTipoDoc=" + sTipoDoc + "&sNroDoc=" + sNroDoc,
    //        success: function (datos) {
    //            console.log(datos);
    //            if (datos === "") {                  
    //                infoCustom("El archivo no se ha descargado. Inténtelo en otro momento.");
