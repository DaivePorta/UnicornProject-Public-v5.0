<%@ WebHandler Language="VB" Class="NOMGNLO" %>

Imports System
Imports System.Web
Imports System.Data
Public Class NOMGNLO : Implements IHttpHandler
   ...
   TIPO_ALMACENAJE, DEPEND_CODE, ESTADO, USUARIO, SISTEMA_ALMACENAJE, PALETIZADO_IND, NRO_PALETS As String
    ...
    Dim P_TXT, P_FASE, P_ALM As String
    Dim P_FECHAREGISTRO, P_TIPOFABRICARION, P_PIDM, P_CANTIDADFABRICACION As String


    Dim P_CODE_FTVOFAB, P_SECCION_ALM, P_FECHAINI_LOTE, P_FECHAFIN_LOTE, P_CANTIDAD_LOTE, P_ESTADO_LOTE As String
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        P_FASE = context.Request("P_FASE")
        P_ALM = context.Request("P_ALM")
        P_FECHAREGISTRO = context.Request("P_FECHAREGISTRO")
        P_TIPOFABRICARION = context.Request("P_TIPOFABRICARION")
        P_PIDM = context.Request("P_PIDM")

        P_CANTIDADFABRICACION = context.Request("P_CANTIDADFABRICACION")

        P_CODE_FTVOFAB = context.Request("P_CODE_FTVOFAB")
        P_SECCION_ALM = context.Request("P_SECCION_ALM")
        P_FECHAINI_LOTE = context.Request("P_FECHAINI_LOTE")
        P_FECHAFIN_LOTE = context.Request("P_FECHAFIN_LOTE")
        P_CANTIDAD_LOTE = context.Request("P_CANTIDAD_LOTE")
        P_ESTADO_LOTE = context.Request("P_ESTADO_LOTE")
        ...
        Try
            Select Case opcion
                Case "1"
                    ...
                    dt = p.ListarSeccionAlmacen(String.Empty, String.Empty, P_ALM, String.Empty)
                ...
                Case "2"
                    ...
                    If Not (dt Is Nothing) Then
                        ...
                        For Each MiDataRow As DataRow In dt.Rows
                            resb.Append("{")
                            resb.Append("""CODIGO"" :" & """" & MiDataRow("CODIGO").ToString & """,")
                            If MiDataRow("ALMC_CODE").ToString = "" Then
                                resb.Append("""ALMACEN"" :" & """N"",")
                            Else
                                resb.Append("""ALMACEN"" :" & """" & MiDataRow("ALMC_CODE").ToString & """,")
                            End If
                ...
                Case "3"
                    dt = c.LISTAR_PROCESAMIENTO_SOLICITUD(CTLG_CODE, p_ESTABLECIMIENTO, P_ESTADO, p_CODIGO, P_FASE)
                    If Not dt Is Nothing Then
                        resb.Append("[")
                ...
                Case "4"
                    dt = c.LISTAR_DETALLE_PROCESAMIENTO(p_CODIGO, P_ESTADO, P_FASE)
                    If Not dt Is Nothing Then
                        resb.Append("[")
                        For Each row As DataRow In dt.Rows
                            resb.Append("{")
                    ...
                    res = resb.ToString()
                Case "5"
                    dt = c.BUSCAR_PROCESAMIENTO_SOLICITUD(CTLG_CODE, p_ESTABLECIMIENTO, Utilities.fechaLocal(P_FECHAINI), Utilities.fechaLocal(P_FECHAFIN))
                    If Not dt Is Nothing Then
                        resb.Append("[")
                        For Each row As DataRow In dt.Rows
                            resb.Append("{")
                ...
                Case "6"
                    ...
                    Dim arr1 As Array
                    Try
                        arr = c.CREAR_ORDEN_FABRICACION(Utilities.fechaLocal(P_FECHAREGISTRO), p_GLOSA, P_ESTADO, P_TIPOFABRICARION, P_PIDM, P_TXT, p_CATALOGO, P_SURCURSAL, P_CANTIDADFABRICACION)
                        c.CREAR_DETALLE_ORD_FABRICACION(P_TXT, arr(1).ToString())
                        arr1 = c.CRAR_LOTE_PRODUCCION(arr(1).ToString(), P_SECCION_ALM, Utilities.fechaLocal(P_FECHAINI_LOTE), Utilities.fechaLocal(P_FECHAFIN_LOTE), P_CANTIDAD_LOTE, P_ESTADO_LOTE)
                        If arr1(2).ToString = "ok" Then
                            resb.Append("[")
                            resb.Append("{")
                            resb.Append("""CORRELATIVO_0"" :" & """" & arr(1).ToString & """,")
                            resb.Append("""CORRELATIVO_1"" :" & """" & arr1(1).ToString & """,")
                            resb.Append("""SUCCESS"" :" & """" & arr1(2).ToString & """")
                            resb.Append("}")
                            resb.Append("]")
                        End If
