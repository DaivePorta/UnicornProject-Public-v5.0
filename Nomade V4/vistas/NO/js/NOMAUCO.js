...
var NOMAUCO = function () {
    ...
    var REGISTRAR = function () {
        ...
        if (oTableActividad.fnGetData().length === 0) {
            alertCustom("Debe agreguar detalle de productos.");
            return;
        }

        if (!vErrors(["cboEmpresas", "cboEstablecimiento", "txtFecha", "txtSolici", "cbPrioridad", "cboRque", "txt_centro_costo", "txtGlosa"])) {
            return;
        }
        ...
        data.append('p_SOLICITA2', $('#ctl00_txtus').val());
        ...
        $.ajax({
            type: "POST",
            url: "vistas/no/ajax/NOMRCOM.ashx?OPCION=REGMIREQ",
            contentType: false,
            data: data,
            processData: false,
            cache: false,
            success: function (datos) {
                ...
                window.location.href = '?f=NOMAUCO&codigo=' + datos;
                $("#idRegis").remove();
            },
            error: function (msg) {
                alertCustom(msg.d);
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== null) {
                        ...
                        if (data[0].AREA !== "") { codigos.push(data[0].AREA) }
                        if (data[0].SECCION !== "") { codigos.push(data[0].SECCION) }
                        if (data[0].PROCESO !== "") { codigos.push(data[0].PROCESO) }
                        if (data[0].ACTIVIDAD !== "") { codigos.push(data[0].ACTIVIDAD) }



                        if (codigos.length > 0) {
                            for (var i = 0; i < codigos.length; i++) {
                                $($("#div_cc select")[i]).select2("val", codigos[i]).change();
                            }
                        }
        ...
        var cuerpo = '<div class="row-fluid">\
                                ...
                                <div class="row-fluid">\
                                    <div class="span12" style="padding: 10px; border: thin inset">\
                                        ...
                                        <h6><strong>SOLICITANTE:</strong> ' + $('#txtSolici').val() + '</h6>\
                                        <h6><strong></strong> </h6>\
