    ...
    var eventoControles = function () {
        ...
        $('#btnFiltrar').click(function () {
            if (vErrors(['cboEmpresa', 'cboMon', 'txtAnio'])) {
                ...
                $.ajax({
                    type: "post",
                    url: 'vistas/NO/ajax/NOLRACM.ashx?OPCION=SUBGRUPOS_COMPRADOS&CTLG_CODE=' + $('#cboEmpresa').val() + '&SCSL_CODE=' + $('#cboSucursal').val() + '&MONE_CODE=' + $('#cboMon').val() + '&ANIO=' + $('#txtAnio').val(),
                    contenttype: "application/json",
                    datatype: "json",
                    success: function (datos) {
                        if (datos !== null) {
                            ...
                            for (var i = 0 ; i < datos.length; i++) {
                                $('#tblReporteMostrar tbody').append('<tr><td style="text-align: center">' + ((i + 1) === datos.length ? 0 : (i + 1)) + '</td><td>' + datos[i].SUBGRUPO + '</td>\
                                ...
                                $('#tblReporte tbody').append('<tr><td>' + datos[i].SUBGRUPO + '</td><td>' + parseFloat(datos[i].MONTO).toFixed(2) + '</td></tr>');
