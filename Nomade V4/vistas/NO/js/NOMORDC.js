...
var NOMORDC = function () {
    ...
    var listarCotizaciones = function () {
        ...
        $.ajax({
            type: "POST",
            url: "vistas/no/ajax/NOMORDC.ashx?OPCION=CT&p_CATALOGO=" + $('#cbo_Empresa').val() + "&P_SUCURSAL=" + $('#cbo_Sucursal').val() + "&P_TIPO_COTI=" + TIPO,
            async: false,
            success: function (datos) {
                Desbloquear("ventana");
                if (datos != null && datos != "" && datos != "[+]" && datos != "[-]") {
                    var json = $.parseJSON(datos)
                    oTableCotiza.fnClearTable()
                    oTableCotiza.fnAddData(json)
                    setTimeout(function () {
                        oTableCotiza.fnAdjustColumnSizing();
                    }, 500);
                }
                else {
                    oTableCotiza.fnClearTable()
            ...
            error: function (msg) {
                Desbloquear("ventana");
                alertCustom('Error al listar Cotizaciones');
            }
        });
    }

    var llenaTabla = function () { // Llena tabla cotizaciones
        var parms = {
            data: null,
            //responsive: true,
            order: [[1, 'desc']],
        ...
        oTableCotiza = iniciaTabla('tblLISTA', parms);
        ...
        $('#tblLISTA tbody').on('click', 'tr', function () {
            if ($(this).hasClass('selected')) {
                ...
                oTableCotiza.$('tr.selected').removeClass('selected');
                $(this).addClass('selected');

                oTableActividad.fnClearTable();
                limpiarDatosMonetarios();

                var pos = oTableCotiza.fnGetPosition(this);
                var row = oTableCotiza.fnGetData(pos);
                var tp = row.CODIGO;

                $('#txtCodigoCotizacion').val(tp);
                $('#modal-cotiza').modal('hide');
            }
        });
    }

    var listarDetalleCotiza = function () {

        Bloquear("ventana");
        $.ajax({
            type: "POST",
            url: "vistas/no/ajax/NOMORDC.ashx?OPCION=CTD&P_CODE=" + $('#txtCodigoCotizacion').val(),
            async: false,
            success: function (datos) {
                Desbloquear("ventana");
                if (datos != null && datos != "" && datos != "[+]") {
                    var json = $.parseJSON(datos)
                    oTableDetalleCotiza.fnClearTable()
                    oTableDetalleCotiza.fnAddData(json)
                    setTimeout(function () {
                        oTableDetalleCotiza.fnAdjustColumnSizing();
                    }, 500);
                }
                else {
                    oTableDetalleCotiza.fnClearTable()
    ...
    var llenaDetalleTabla = function () { // Llena tabla detalles de cotizacion
        var parms = {
            data: null,
            responsive: true,
            order: [[1, 'desc']],
        ...
        $('#tblDetalleCotiza').dataTable().fnDestroy();

        oTableDetalleCotiza = iniciaTabla('tblDetalleCotiza', parms);

        $('#tblDetalleCotiza tbody').on('click', 'tr', function () {
            if ($(this).hasClass('selected')) {
                $(this).removeClass('selected');
            }
            else {
                oTableDetalleCotiza.$('tr.selected').removeClass('selected');
                $(this).addClass('selected');

                limpiar();

                var pos = oTableDetalleCotiza.fnGetPosition(this);
                var row = oTableDetalleCotiza.fnGetData(pos);
                var tp = row.CODE_PRODUCTO;

                var ar = oTableActividad.fnGetData();
                var flag = false;
                ar.filter(function (e, f) {
                    if (e.CODIGO == row.CODE_PRODUCTO) {
                        alertCustom("Producto ya ha sido agregado.")
                        flag = true;
                    }
                });

                if (!flag) {
                    $('#txtcodprod').val(row.CODE_PRODUCTO);
                    $('#txtdescprod').val(row.PRODUCTO);
                    $('#txtcant').val(row.CANTIDAD);
                    $('#txtUnidad').val(row.UNIDMEDIDAD);
                    $("#hdcodUNI").val(row.CODE_UNIDAD)
                    $('#modal-detalle-cotiz').modal('hide');
                    $("#hfporcentaje_detraccion").val(row.DETRACCION)
                }
            }
        });
    }
    ...
    var REGISTRAR = function () {
        ...
        data.append('p_SOCOTI_CODE', $('#txtCodigoCotizacion').val());
        ...
        $.ajax({
            type: "POST",
            url: "vistas/no/ajax/NOMORDC.ashx?OPCION=13",
            contentType: false,
            data: data,
            processData: false,
            cache: false,
                ...
                if (datos[0].error != "error") {
                    if (datos[0].NOCORREO != "NOCORREO") {
                        if (datos[0].SUCCESS == "OK") {
                            ...
                            //$("#lbl_estado").html("&nbsp;&nbsp;ESTADO&nbsp;:&nbsp;GENERADO");
                            //$("#btn_cancel_orden").css("display", "");
                            $("#idRegis").hide();
                            $("#idModificar").show();
    ...
    var MODIFICAR = function () {
        ...
        $.ajax({
            type: "POST",
            url: "vistas/no/ajax/NOMORDC.ashx?OPCION=14",
            contentType: false,
            data: data,
            processData: false,
            cache: false,
    ...
    var completarDocumento = function () {
        ...
        data.append('p_MONETAR_PREC_TOTAL', $('#txt_prec_total').val());
        data.append('p_GLOSA_GENERAL', $('#txtGlosa_general').val());
        data.append('p_SOCOTI_CODE', $('#txtCodigoCotizacion').val());//txtCodigoCotizacion
        ...
        $.ajax({
            type: "POST",
            url: "vistas/no/ajax/NOMORDC.ashx?OPCION=15",
            contentType: false,
            data: data,
            processData: false,
            cache: false,
    ...
    var datosTabla = function () {
        ...
        $('#detalle tbody').children().each(function (i) {
            ...
            var v_PRODUCTO, v_UNID_MEDIDAD, v_CANTIDAD, v_IMPORTE, v_GLOSA, ITEMS, v_PREC_UNI, v_DESC_PROD;

            v_PRODUCTO = row.CODIGO;
            v_DESC_PROD = row.DES_PRODUCTO;
            ...
            datos_fila += v_PRODUCTO + ',' + v_DESC_PROD + ',' + v_UNID_MEDIDAD + ',' + v_CANTIDAD + ',' + v_PREC_UNI + ',' + v_IMPORTE + ',' + v_GLOSA + ',' + $('#txtCodigoCotizacion').val();
            datos_fila += '|';
        });
    ...
    //            $('#cboCorreos').empty();
    //            $('#cboCorreos').append('<option></option>');
    //            if (datos != null) {
    //                var nombres = '';
    //                for (var i = 0; i < datos.length; i++) {
    ...
    //                    } else {
    //                        $('#cboCorreos').append('<option value="' + datos[i].Correo + '">' + datos[i].Nombres + ' [ ' + datos[i].Correo + ' ]' + '</option>');
    //                    }
    //                }
    //            }
    //        },
    ...
    var eventoControles = function () {
        ...
        $('#btnDetalle').click(function () {
            if (vErrors("txtCodigoCotizacion")) {
                listarDetalleCotiza();
                $('#modal-detalle-cotiz').modal('show');
            }
        });
        ...
        $('#cbo_modo_pago').on('change', function () {
            if ($('#cbo_modo_pago').val() == "0002") {
                $("#txt_plazo_pago").attr("disabled", false);
                $("#txt_fec_vencimiento").attr("disabled", false);
        ...
        $('#idRegis').click(function () {
            ...
            if (vErrors(['txt_proveedor', 'txtCodigoCotizacion'])) {
                if (oTableActividad.fnGetData().length == 0) {
                    alertCustom("Agrege Bienes / Servicios")
                }
                else {
        ...
        $("#btn_cancel_orden").click(function () {
    ...
    var CancelarOrden = function () {

        var cod = $("#txtNumDctoComp").val();
        var detalle = datosTablaCancelar();

        $.ajax({
            type: "post",
            url: "vistas/no/ajax/NOMORDC.ashx?OPCION=20&P_CODIGOCAB=" + cod + "&p_DETALLE_ORD=" + detalle,
            //contenttype: "application/json;",
            //datatype: "json",
            async: false,
            success: function (data) {
                ...
                if (!isEmpty(data)) {
                    ...
                    if (data == "PAGADO") {
                        alertCustom("No se puede realizar la cancelacion de la orden, debido que la deuda que genero ya fue pagado!")
                        return
                    }
                    if (data == "E") {
                        noexito();
                    ...
                    if (data == "OK") {
                        $("#lbl_estado").html("&nbsp;&nbsp;ESTADO&nbsp;:&nbsp;CANCELADO");
                        $("#btn_cancel_orden").remove();
                        exito();
                    }
    ...
    var datosTablaCancelar = function () {
        ...
        if (cod != undefined) {
            $('#tblbmodal tbody').children().each(function (i) {

                var v_PRODUCTO, v_UNID_MEDIDAD, v_CANTIDAD, v_IMPORTE, v_GLOSA, ITEMS, v_PREC_UNI;

                v_PRODUCTO = $(this).find('td').eq(0).text();
                v_UNID_MEDIDAD = $(this).find('td').eq(3).text();
                v_CANTIDAD = $(this).find('td').eq(4).text();
                v_PREC_UNI = $(this).find('td').eq(5).text();
                v_IMPORTE = $(this).find('td').eq(6).text();
                v_GLOSA = $(this).find('td').eq(7).text();
                ...
                datos_fila += v_PRODUCTO + ',' + v_UNID_MEDIDAD + ',' + v_CANTIDAD + ',' + v_PREC_UNI + ',' + v_IMPORTE + ',' + v_GLOSA + ',' + $('#txtCotizacion1').val();
                datos_fila += '|';
            });
        } else {
            $('#detalle tbody').children().each(function (i) {
    ...
    var listarDetalleModificar = function (detalle) {
        Bloquear("div");
        $.ajax({
            type: "POST",
            url: "vistas/no/ajax/NOMORDC.ashx?OPCION=7&P_CODIGOCAB=" + detalle,
            async: false,
            success: function (datos) {
                    ...
                    if (data !== null) {
                        ...
                        if (data[0].COMPLETO_IND === 'S'){
                            ...
                            if (data[0].INCL_IGV_ITEMS == 'S') {
                                igvitems = " * Precios de Items incluye IGV.";
                            }
                            else {
                                igvitems = " * Precios de Items no incluye IGV. ";
                            }
                            ...
                            if (data[0].IND_CANCELADO == "S") {
                                $("#lbl_estado").html("&nbsp;&nbsp;ESTADO&nbsp;:&nbsp;CANCELADO");
                                $("#btn_cancel_orden").remove();
                            }
                            if (data[0].IND_CANCELADO == "N") {
                                $("#lbl_estado").html("&nbsp;&nbsp;ESTADO&nbsp;:&nbsp;GENERADO");
                                $("#btn_cancel_orden").css("display", "");
                            }

                            if (data[0].IND_CANCELADO == "P") {
                                $("#lbl_estado").html("&nbsp;&nbsp;ESTADO&nbsp;:&nbsp;COMPLETADO");
                                $("#btn_cancel_orden").css("display", "");
                            }
                            $("#hfPIDM").val(data[0].PIDM_PROVEEDOR);
                            correo(data[0].PIDM_PROVEEDOR)
                            $($("#p_monetarios").children().children()[0]).children()[1].remove();
                            $("#idModificar").hide();
                            $("#idCompletar").hide();
                            $("#divMsgCorreo").css("display", "block");
                            ...
                            listarDetalleModificar(data[0].CODIGO);

                            $('#cbo_moneda').attr('disabled', false);
                            $('#txt_glosa_dt').attr('disabled', false);
                            
                            $("#s2id_autogen1_search").attr('disabled', false)

                            //$('#Cotizac').css('display', 'block');

                            $('#txtCodigoCotizacion').val(data[0].COTIZACION);
...
function AgregarDetalle() {
    ...
    if (vErrors(['txtcant', 'txt_importe', 'txtcodprod', 'txtdescprod', 'txtUnidad'])) {
...
function datosImporte() {
    ...
    $('#detalle tbody').children().each(function (i) {
        CANTIDAD += parseFloat($(this).find('td').eq(5).text());
        ITEMS = i + 1;
    });
...
function datosDetraccion() {
    ...
    var CANTIDAD = 0, ITEMS;
    $('#detalle tbody').children().each(function (i) {
        CANTIDAD += parseFloat($(this).find('td').eq(7).text());
        ITEMS = i + 1;
    });
    datos_tabla = CANTIDAD;
    return datos_tabla;
