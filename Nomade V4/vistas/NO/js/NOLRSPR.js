var NOLRSPR = function () {
    ...
    var eventoControles = function () {
        $('#cboEstado').change(function () {
            $('#tblProductos').DataTable().column(6).search($(this).val()).draw();
        });
    ...
    var listar = function () {
        ...
        $("#tblProductos").DataTable({
            ajax: {
                type: "POST",
                url: "vistas/no/ajax/NOMRSPR.ashx?OPCION=8",
                async: false,
        ...
        $('#tblProductos tbody').on('click', 'tr', function () {
            if ($(this).hasClass('selected')) {
                ...
                window.location.href = '?f=NOMRSPR&codigo=' + codigo;
            }
        });
    };
