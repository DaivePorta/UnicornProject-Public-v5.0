...
var NSLSOHO = function () {
    ...
    let cargainicial = () => fillCboEmpresa();
    ...
    let handleTablaSolicitud = function () {
        ...
        $('#tblHoraExtra tbody').on('click', 'tr', function () {
            if ($(this).hasClass('selected')) {
                ...
                window.location.href = '?f=NSMSOHO&Opcion=D&codigo=' + codigo + "&Peri=" + Peri + "&Emp=" + Emp;
            }
...
;
    ...
    let ListarSolicitud = () => {
        ...
        $.ajax({
            type: "POST",
            url: "vistas/NS/ajax/NSLAPHO.ashx?Opcion=" + "X" + "&Emp=" + Emp + "&Suc=" + Suc + "&PIDM=" + empleadoPIDM + "&MES=" + mes + "&ANIO=" + aa,
            contentType: "application/json;",
            dataType: "json",
            success: function (datos) {
        ...
        if (codigo != null) {
            ...
            $.ajax({
                ...
                success: function (datos) {
                    ...
                    $('#btnNuevo').attr({ 'href': '?f=NSMSOHO' });
                    $('#btnCancelar').attr({ 'href': '?f=NSMSOHO' });
