var NELREGG = function () {
    ...
    var eventoControles = function () {
        ...
        $('#btnFiltrar').click(function () {
            if (vErrors(['cboEmpresa'])) {
                ...
                $.ajax({
                    type: "post",
                    url: 'vistas/NE/ajax/NELREGG.ashx?OPCION=EGRESOS_GENERALES&CTLG_CODE=' + $('#cboEmpresa').val() + '&SCSL_CODE=' + $('#cboSucursal').val() + '&DESDE=' + desde + '&HASTA=' + hasta + "&CECC_CODE=" + ($("#txt_centro_costo").data("CodCentroCostoCab")||"") + '&CECD_CODE=' + ($("#txt_centro_costo").data("CodCentroCosto")||"") ,
                    contenttype: "application/json",
                    datatype: "json",
                    success: function (datos) {
                        if (datos !== null) {
                            ...
                            for (var i = 0 ; i < datos.length; i++) {
                                ...
                                $('#tblReporte tbody').append('<tr><td>' + datos[i].CONCEPTO + '</td><td>' + datos[i].MONTO + '</td></tr>');
                        ...
                        $('#chart').highcharts({
                            ...
                            title: { text: 'Analítico de Egresos por Concepto' },
                            legend: {
                                enabled: false
                            },
                            yAxis: {
                        ...
                        $('#pie').highcharts({
                            ...
                            title: { text: 'Torta de Analítico de Egresos Generales' },
                            tooltip: {
                                formatter: function () {
    ...
    var funcionalidadTabla = function () {
        ...
        $('#tblReporteMostrar tbody').on('click', 'tr', function () {
            ...
            $.ajax({
                type: "post",
                url: 'vistas/NE/ajax/NELREGG.ashx?TIPO=C&OPCION=MOSTRAR_DETALLE&CTLG_CODE=' + $('#cboEmpresa').val() + '&SCSL_CODE=' + $('#cboSucursal').val() + '&DESDE=' + desde + '&HASTA=' + hasta + '&CONC_CODE=' + ID + "&CECC_CODE=" + ($("#txt_centro_costo").data("CodCentroCostoCab")||"") + '&CECD_CODE=' + ($("#txt_centro_costo").data("CodCentroCosto")||"") ,
                contenttype: "application/json;",
                datatype: "json",
                async: true,
                success: function (datos) {
