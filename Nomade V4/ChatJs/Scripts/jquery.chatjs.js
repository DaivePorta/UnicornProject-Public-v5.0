    ...
    ChatContainer.prototype = {
        ...
        init: function () {
            ...
            if (_this.opts.canClose) {
                ...
                var $clearButton = $("<div/>").addClass("clearChat").appendTo(_this.$windowTitle);
                ...
                $clearButton.click(function (e) {
                    ...
                    // triggers the event
                    _this.opts.adapter.server.removeMensajesChat(_this.opts.myUser.Id, _this.opts.otherUser.Pidm, 666);

                    _this.$windowInnerContent.html('');
...
(function ($) {
    ...
    function ChatWindow(options) {
        ...
        this.addMessage = function (message, clientGuid) {
            ...
            if (message.UserFrom.Pidm != this.opts.myUser.Id) {
            ...
            if (message.ClientGuid && $("p[data-val-client-guid='" + message.ClientGuid + "']").length) {
                ...
                var $fechaMessage = new Date(message.Timestamp);
                var $fechaActual = new Date();
                var $fechaMostrar;

                if ($fechaMessage.getDate() == $fechaActual.getDate()) {
                    if ($fechaMessage.getHours() > 12) {
                        $fechaMostrar = Math.abs($fechaMessage.getHours() - 12).toString() + ':' + $fechaMessage.getMinutes() + ' pm';
                    } else {
                        $fechaMostrar = $fechaMessage.getHours().toString() + ':' + $fechaMessage.getMinutes() + ' am';
                    }

                } else {
                    $fechaMostrar = $fechaMessage.getDate().toString() + '/' + ($fechaMessage.getMonth() + 1).toString() + '/' + $fechaMessage.getFullYear().toString() + Math.abs($fechaMessage.getHours() - 12).toString() + ':' + $fechaMessage.getMinutes();
                }

                var $messageP = $("<p/>").text(message.Message);
                var $messageDate = $("<div/>").text($fechaMostrar).addClass("date-message");
                ...
                if ($lastMessage.length && $lastMessage.attr("data-val-user-from") == message.UserFrom.Pidm) {
                    // we can just append text then
                    $messageP.appendTo($(".chat-text-wrapper", $lastMessage));
                    $messageDate.appendTo($(".chat-text-wrapper", $lastMessage));
                }
                else {
                    // in this case we need to create a whole new message
                    var $chatMessage = $("<div/>").addClass("chat-message").attr("data-val-user-from", message.UserFrom.Pidm);
                    ...
                    //add Date
                    $messageDate.appendTo($textWrapper);

                    // add image
                    $("<img/>")
                        .attr("src", decodeURI(message.UserFrom.Foto))
        ...
        this.sendMessage = function (messageText) {
            ...
            _this.opts.adapter.server.sendMessage(_this.opts.otherUser.Pidm, messageText, clientGuid);
        };

        this.sendTypingSignal = function () {
            /// <summary>Sends a typing signal to the other user</summary>
            var _this = this;
            _this.opts.adapter.server.sendTypingSignal(_this.opts.otherUser.Pidm);
        };
    ...
    ChatWindow.prototype = {
        ...
        init: function () {
            ...
            _this.chatContainer = $.chatContainer({
                ...
                canClose: true,
                myUser: _this.opts.myUser,
                otherUser: _this.opts.otherUser,
            ...
            _this.chatContainer.setTitle(_this.opts.otherUser.Usuario);

            _this.opts.adapter.server.getMessageHistory(_this.opts.otherUser.Pidm, function (messageHistory) {
        ...
        showTypingSignal: function (user) {
            ...
            _this.$typingSignal = $("<p/>").addClass("typing-signal").text(user.Usuario + _this.opts.typingText);
...
(function ($) {
    ...
    function Chat(options) {
        ...
        _this.defaults = {
            ...
            adapter: null,
            titleText: 'Chat',
            emptyRoomText: "No hay usuarios conectados",
        ...
        _this.createNewChatWindow = function (otherUser, initialToggleState, initialFocusState) {
            ...
            var newChatWindow = $.chatWindow({
                ...
                onClose: function () {
                    delete _this.chatWindows[otherUser.Pidm];
                    $.organizeChatContainers();
                    _this.saveWindows();
                },
                onToggleStateChanged: function (toggleState) {
            ...
            // this cannot be in t
            _this.chatWindows[otherUser.Pidm.toString()] = newChatWindow;
            _this.saveWindows();
        };
        ...
        _this.processUserListAjaxResult = function (data) {
            ...
            if (data.length <= 1) {
                ...
                for (var i = 0; i < data.length; i++) {
                    if (data[i].Pidm != _this.opts.user.Id) {
                        indexedData[data[i].Pidm] = data[i];
                        var $userListItem = $("<div/>")
                            .addClass("user-list-item")
                            .attr("data-val-id", data[i].Pidm)
                            .appendTo(_this.chatContainer.getContent());

                        $("<img/>")
                            .addClass("profile-info")
                            .addClass("profile-picture")
                            .attr("src", data[i].Foto)
                            .appendTo($userListItem)
                            .attr("width", "25px")
                            .attr("height", "25px")
                            .attr("data-nombre", data[i].Nombre)
                            .attr("data-ctlg", data[i].Catalogo)
                            .attr("data-scsl", data[i].Sucursal)
                            .attr("data-ip", data[i].IP)
                            .attr("data-navegador", data[i].Navegador);
                        ...
                        (function (otherUserId) {
                            ...
                            $userListItem.click(function () {
                                if (_this.chatWindows[data[otherUserId].Pidm]) {
                                    _this.chatWindows[data[otherUserId].Pidm].focus();
                                } else
                                    _this.createNewChatWindow(data[otherUserId]);
                            });

                            if (_this.opts.user.Name == 'ADMINSIS') {
                                $(".profile-info").mouseover(function () {
                                    $("#user_nombre").text($(this).attr("data-nombre"));
                                    $("#user_empresa").text($(this).attr("data-ctlg"));
                                    $("#user_sucursal").text($(this).attr("data-scsl"));
                                    $("#user_navegador").text($(this).attr("data-navegador"));
                                    $("#user_ip").text($(this).attr("data-ip"));
                                    $("#info_user_chat").show();
                                });

                                $(".profile-info").mouseout(function () {
                                    $("#info_user_chat").hide();
                                });
                            }
                        })(i);
    ...
    Chat.prototype = {
        ...
        init: function () {
            ...
            _this.client.sendMessage = function (message) {
                ...
                if (message.UserFrom.Pidm != _this.opts.user.Id) {
                    // in this case this message did not came from myself
                    if (!_this.chatWindows[message.UserFrom.Pidm])
                        _this.createNewChatWindow(message.UserFrom);
                    else
                        _this.chatWindows[message.UserFrom.Pidm].addMessage(message);
                    ...
                    if (_this.chatWindows[message.UserTo.Pidm]) {
                        _this.chatWindows[message.UserTo.Pidm].addMessage(message);
                    }
                }
            };
            ...
            _this.client.sendTypingSignal = function (otherUser) {
                ...
                if (_this.chatWindows[otherUser.Pidm]) {
                    _this.chatWindows[otherUser.Pidm].showTypingSignal(otherUser);
                }
            };
